        ...
        if (particle -> Mother() == 0 ){ // then this is a primary
          ...
          if (particle -> PdgCode() == 111)  // neutral pion
          {
            ...
            if ( particle -> NumberDaughters() == 2){
              TLorentzVector conversionPoint, conversionMom;
              art::Ptr<simb::MCParticle> daughter0 = getParticleByID(mclarg4,particle->Daughter(0));
              ...
              art::Ptr<simb::MCParticle> daughter1 = getParticleByID(mclarg4,particle->Daughter(1));
              ...
              for (int daughter = 0; daughter < 3; daughter ++ ){
                art::Ptr<simb::MCParticle> daughterParticle = getParticleByID(mclarg4,particle->Daughter(daughter));
                if (daughterParticle->PdgCode() == 22) {
                  GetPhotonConversionInfo(daughterParticle, conversionPoint,conversionMom);
                  p1PhotonConversionPos.push_back(conversionPoint);
                  p1PhotonConversionMom.push_back(conversionMom);
                  break;
                }
