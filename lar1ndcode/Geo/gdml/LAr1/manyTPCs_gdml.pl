...
sub gen_wireplane()
{
  ...
  # The number of full-length "center" wires.
   ...
   if ( $wires_on == 1 )
	  { $NumberCenterWires = 1455 - 2*$NumberWiresPerEdge ; }
...
#This goes inside volCryostat
	...
	<physvol>
		...
		<position name="posTPC1" unit="cm" x="$TPCWidth/2+2.5" y="0" z="$TPCLength/4" />
	</physvol>
	<physvol>
		<volumeref ref="volTPC"/>
		<position name="posTPC2" unit="cm" x="$TPCWidth/2+2.5" y="0" z="-$TPCLength/4" />
	</physvol>
	<physvol>
		<volumeref ref="volTPC"/>
	...
	<physvol>
		<volumeref ref="volTPC"/>
		<position name="posTPC4" unit="cm" x="-$TPCWidth/2-2.5" y="0" z="-$TPCLength/4" />
		<rotationref ref="rPlus180AboutY"/> 
	</physvol>  
