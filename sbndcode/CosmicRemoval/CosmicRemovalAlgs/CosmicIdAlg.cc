...
bool CosmicIdAlg::CosmicId(recob::Track track, const art::Event& event, std::vector<double> t0Tpc0, std::vector<double> t0Tpc1){
  ...
  if(fApplyStoppingCut){
    ...
    if(spTag.StoppingParticleCosmicTag(track, calos)) return true;
  }

  if(fApplyGeometryCut){
    bool tpc0Flash = CosmicRemovalUtils::BeamFlash(t0Tpc0, 4.);
    bool tpc1Flash = CosmicRemovalUtils::BeamFlash(t0Tpc1, 4.);
    if(geoTag.GeometryCosmicTag(track, hits, tpc0Flash, tpc1Flash)) return true;
  }
  ...
  if(fApplyCpaCrossCut){
    ...
    if(ccTag.CpaCrossCosmicTag(track, tracks, findManyHits)) return true;
  }

  if(fApplyApaCrossCut){
    if(acTag.ApaCrossCosmicTag(track, hits, t0Tpc0, t0Tpc1)) return true;
  }
...
bool CosmicIdAlg::CosmicId(recob::PFParticle pfparticle, std::map< size_t, art::Ptr<recob::PFParticle> > pfParticleMap, const art::Event& event, std::vector<double> t0Tpc0, std::vector<double> t0Tpc1){
  ...
  if(fApplyPandoraT0Cut){
    if(ptTag.PandoraT0CosmicTag(pfparticle, pfParticleMap, event)) return true;
  }
  ...
  if(secondaryTracks.size() > 0){
    ...
    if(secondaryTracks[0].second > fMinMergeAngle){
      ...
      if(fApplyStoppingCut){
        ...
        if(spTag.StoppingParticleCosmicTag(track, track2, calos, calos2)) return true;
      }
      ...
      if(fApplyApaCrossCut){
        ...
        if(acTag.ApaCrossCosmicTag(track2, hits2, t0Tpc0, t0Tpc1)) return true;
      }
