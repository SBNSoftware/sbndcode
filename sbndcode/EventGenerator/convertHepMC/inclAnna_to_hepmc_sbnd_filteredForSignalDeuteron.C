...
void inclAnna_to_hepmc_sbnd_filteredForSignalDeuteron(std::string indir , std::string inclxx_file_name , std::string outdir, int eventsperfile=100) {
   ...
   while (et_reader.Next() && at_reader.Next()){
      ...
      */
      ...
      bool containsSignalDeuteron = 0;

      for (size_t j = 0; j < *nParticles; j++) {

         if(pdg[j] == 1002 && Ekin[j] > 64.44){
            containsSignalDeuteron = 1;
         }

      }

      if(!containsSignalDeuteron)
      continue;
