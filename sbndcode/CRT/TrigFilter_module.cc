...
namespace filt{
  ...
  void TrigFilter::reconfigure(fhicl::ParameterSet const& pset)
  {
    ...
    fmodlistUTopL = pset.get<std::vector<int>>("ModuleListUpstreamTopLeft");
    fmodlistUBotL = pset.get<std::vector<int>>("ModuleListUpstreamBotLeft");
    fmodlistUTopR = pset.get<std::vector<int>>("ModuleListUpstreamTopRight");
    fmodlistUBotR = pset.get<std::vector<int>>("ModuleListUpstreamBotRight");
    fmodlistDTopL = pset.get<std::vector<int>>("ModuleListDownstreamTopLeft");
    fmodlistDBotL = pset.get<std::vector<int>>("ModuleListDownstreamBotLeft");
    fmodlistDTopR = pset.get<std::vector<int>>("ModuleListDownstreamTopRight");
    fmodlistDBotR = pset.get<std::vector<int>>("ModuleListDownstreamBotRight");
    fstripmatch = pset.get<bool>("RequireStripMatch",true);
    fstripshift = pset.get<int>("AllowedStripShift",4);
    fedgecut = pset.get<int>("EdgeCutInStrips",12);
    if (fedgecut>31) fedgecut=31;
    fTimeCoinc = pset.get<float>("StripTimeCoincidence",0.2);
    fADCthresh = pset.get<float>("ADCthresh",500.0);
  }
