...
namespace sbnd{
  ...
  CRTTrackMatchAlg(const fhicl::ParameterSet& pset, geo::GeometryCore const* GeometryService) :
    ...
    std::pair<geo::Point_t, geo::Point_t> TpcIntersection(const geo::TPCGeo& tpcGeo, sbnd::crt::CRTTrack track);

    // Function to calculate if a CRTTrack crosses the TPC volume
    bool CrossesTPC(sbnd::crt::CRTTrack track);

    // Function to calculate if a CRTTrack crosses the TPC volume
    bool CrossesAPA(sbnd::crt::CRTTrack track);
    ...
    std::vector<sbnd::crt::CRTTrack> AllPossibleCRTTracks(detinfo::DetectorPropertiesData const& detProp,
    ...
    std::pair<sbnd::crt::CRTTrack, double> ClosestCRTTrackByAngle(detinfo::DetectorPropertiesData const& detProp,
                                                                  recob::Track tpcTrack,
                                                                  std::vector<sbnd::crt::CRTTrack> crtTracks,
                                                                  const art::Event& event,
                                                                  double minDCA = 0.);
    ...
    std::pair<sbnd::crt::CRTTrack, double> ClosestCRTTrackByDCA(detinfo::DetectorPropertiesData const& detProp,
                                                                recob::Track tpcTrack,
                                                                std::vector<sbnd::crt::CRTTrack> crtTracks,
                                                                const art::Event& event,
                                                                double minAngle = 0.);
    std::pair<sbnd::crt::CRTTrack, double> ClosestCRTTrackByDCA(detinfo::DetectorPropertiesData const& detProp,
                                                                ...
                                                                double minAngle = 0.);
    // Find the closest matching crt track by average DCA between tracks within angle and DCA limits
    std::pair<sbnd::crt::CRTTrack, double> ClosestCRTTrackByScore(detinfo::DetectorPropertiesData const& detProp,
                                                                  recob::Track tpcTrack,
                                                                  std::vector<sbnd::crt::CRTTrack> crtTracks,
                                                                  const art::Event& event);
    std::pair<sbnd::crt::CRTTrack, double> ClosestCRTTrackByScore(detinfo::DetectorPropertiesData const& detProp,
    ...
    double AngleBetweenTracks(recob::Track tpcTrack, sbnd::crt::CRTTrack crtTrack);

    // Calculate the average DCA between tracks
    double AveDCABetweenTracks(recob::Track tpcTrack, sbnd::crt::CRTTrack crtTrack, double shift);
