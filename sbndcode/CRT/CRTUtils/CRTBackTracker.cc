...
namespace sbnd
{
  ...
  bool CRTBackTracker::FEBDataCompare(const sbnd::crt::FEBData &data1, const sbnd::crt::FEBData &data2)
  {
    ...
    if (data1.Coinc() != data2.Coinc())
      return false;
    if (data1.Mac5() != data2.Mac5())
      return false;
    if (data1.Ts0() != data2.Ts0())
      return false;
    if (data1.Ts1() != data2.Ts1())
      return false;
    if (data1.ADC() != data2.ADC())
      return false;
  ...
  bool CRTBackTracker::TrackCompare(const sbn::crt::CRTTrack &track1, const sbn::crt::CRTTrack &track2)
  {
    ...
    if (track1.ts1_ns != track2.ts1_ns)
      return false;

    if (track1.plane1 != track2.plane1)
      return false;
    if (track1.x1_pos != track2.x1_pos)
      return false;
    if (track1.y1_pos != track2.y1_pos)
      return false;
    if (track1.z1_pos != track2.z1_pos)
      return false;
    if (track1.x1_err != track2.x1_err)
      return false;
    if (track1.y1_err != track2.y1_err)
      return false;
    if (track1.z1_err != track2.z1_err)
      return false;

    if (track1.plane2 != track2.plane2)
      return false;
    if (track1.x2_pos != track2.x2_pos)
      return false;
    if (track1.y2_pos != track2.y2_pos)
      return false;
    if (track1.z2_pos != track2.z2_pos)
      return false;
    if (track1.x2_err != track2.x2_err)
      return false;
    if (track1.y2_err != track2.y2_err)
      return false;
    if (track1.z2_err != track2.z2_err)
      return false;
  ...
  std::vector<int> CRTBackTracker::AllTrueIds(const art::Event &event, const sbnd::crt::FEBData &data)
  {
    ...
    {
      if (FEBDataCompare(crtData, data))
        data_i = index;
      index++;
    }
  ...
  int CRTBackTracker::TrueIdFromTotalEnergy(const art::Event &event, const sbnd::crt::FEBData &data)
  {
    ...
    {
      if (FEBDataCompare(febData, data))
        data_i = index;
      index++;
    }
  ...
  int CRTBackTracker::TrueIdFromTotalEnergy(const art::Event &event, const sbn::crt::CRTTrack &track)
  {
    ...
    for (auto const &crtTrack : (*crtTrackHandle))
    {
      if (TrackCompare(crtTrack, track))
        track_i = index;
      index++;
    }
  ...
  CRTBackTracker::TruthMatchMetrics CRTBackTracker::TruthMatrixFromTotalEnergy(const art::Event &event, const art::Ptr<sbn::crt::CRTHit> &crtHit)
  {
    ...
    int pdg = -99999;
    double particle_energy = -99999.;
    double deposited_energy_track = -99999.;
    double time = -99999.;
    for (auto const [id, en] : idToEnergyMap)
    {
      double pur = en / totalEnergy;
      if (pur > bestPur)
      { // id == std::numeric_limits<int>::min() when GEANT4 dropped the track.
        trackid_true = id;
