void BasicVariables()
{
  const TString saveDir = "/exp/sbnd/data/users/hlay/crt/clustering/merge_checks_jan2024/plots/rockbox/basicvariables";
  ...
  std::vector<plt> plots = { {"n_strip_hits", "@sh_channel.size()", ";nStripHits;Events",
                             ...
                             {"cl_completeness_log","cl_truth_completeness", ";Completeness;Clusters",
                              50, 0, 1 + std::numeric_limits<double>::epsilon(), plotcolour, {}, "", true, true},
                             ...
                             {"tr_completeness","tr_truth_completeness", ";Completeness;Tracks",
                              50, 0, 1 + std::numeric_limits<double>::epsilon(), plotcolour},
                             {"tr_completeness_log","tr_truth_completeness", ";Completeness;Tracks",
                              50, 0, 1 + std::numeric_limits<double>::epsilon(), plotcolour, {}, "", true, true},
                             {"tr_purity","tr_truth_purity", ";Purity;Tracks",
                              50, 0, 1 + std::numeric_limits<double>::epsilon(), plotcolour},
                             {"tr_purity_log","tr_truth_purity", ";Purity;Tracks",
                              50, 0, 1 + std::numeric_limits<double>::epsilon(), plotcolour, {}, "", true, true},
  };
