...
private:
    ...
    std::vector<TProfile*>      fHitEfficPHADCdetPropsVec;
    std::vector<TProfile*>      fHitEfficPHADCsigServicesVec;
...
void TrackHitEfficiencyAnalysis::initializeHists(art::ServiceHandle<art::TFileService>& tfs, const std::string& dirName)
{
    ...
    fHitEfficPHADCsigServicesVec.resize(fGeometry->Nplanes());
    fHitEfficPHADCdetPropsVec.resize(fGeometry->Nplanes());
	...
	fHitEfficPHADCdetPropsVec.at(plane)       = dir.make<TProfile>  (("HitEfficPH-detprop"  + std::to_string(plane)).c_str(), "Hit Efficiency;ADC",          50,  0.,  100., 0., 1.);
	fHitEfficPHADCsigServicesVec.at(plane)    = dir.make<TProfile>  (("HitEfficPH-sigServices"  + std::to_string(plane)).c_str(), "Hit Efficiency;ADC",          50,  0.,  100., 0., 1.);
...
void TrackHitEfficiencyAnalysis::fillHistograms(const art::Event& event) const
{
	...
	//	if (fabs(trackPDGCode) != 11 || processName != "primary") continue;
        ...
        // Looking for electrons or muons
	if ( ( fabs(trackPDGCode) != 11 && fabs(trackPDGCode) != 13 ) || processName != "primary") continue;
	    ...
	    float etoADCsigServices = 13198.*1.6*10e-7;
	    float etoADCdetProps    = 6.29778e-3;
            ...
            fHitEfficPHADCsigServicesVec[plane]->Fill(maxElectrons*etoADCsigServices,   matchHit,   1.);
            fHitEfficPHADCdetPropsVec[plane]->Fill(maxElectrons*etoADCdetProps,   matchHit,   1.);
