...
private:
    ...
    std::string fBNBPOTDataLabel;
    std::string fOffbeamBNBCountDataLabel;
    ...
    // More initializers here.
{
    ...
    fBNBPOTDataLabel = p.get<std::string>("BNBPOTDataLabel", "bnbinfo");
    fOffbeamBNBCountDataLabel = p.get<std::string>("OffbeamBNBCountDataLabel", "bnbextinfo");
    ...
    fDebugFilter = p.get<bool>("DebugFilter", false);
    ...
    fApplyFilter = p.get<bool>("ApplyFilter", true);
}
...
void sbnd::BeamAnalysis::analyze(art::Event const& e)
{
    ...
    if (fIncludeCrtSP){
        ...
        if (!crtSpacePointHandle.isValid() || crtSpacePointHandle->size() == 0){
            ...
            for (auto const& crt_sp: crt_sp_v){
                ...
                //Only Save North and South tagger
                if ((crt_cluster->Tagger() == 1) | (crt_cluster->Tagger() == 2)){
                ...
                //// Get associated TPC Track -- only save have spacepoint has a valid TPC track match
    ...
    if (fIncludeCrtTrack){
        ...
        if (!crtTrackHandle.isValid() || crtTrackHandle->size() == 0){
            ...
            for (auto const& crt_trk: crt_trk_v){
                ...
                // Get associated TPC Track -- only save have track has a valid TPC track match
    ...
    if (fIncludePmt){
        ...
        for (size_t s = 0; s < fOpFlashLabels.size(); s++) {
            ...
            if(!opFlashHandle.isValid() || opFlashHandle->size() == 0){
                ...
                //double _frame_apply_at_caf_us = _frame_apply_at_caf / 1000.0; //convert ns to us
    ...
    //Apply filter to reduce filesize -- only fill Tree if pass filter

    if (fApplyFilter){

        //Make sure has a valid shifting frame
        bool passFrame = false;
        if ((_frame_apply_at_caf != 0) | (_frame_apply_at_caf != 1738)) passFrame = true;

        //Has CRT SP within [-800, 2000]
        bool passCrtSP = false;
        int nSPInWindow = 0;
        std::vector<double> crt_sp_ts0_in_window;
        if (passFrame){
            for (auto const& ts0: _crt_sp_ts0){
                double ts0_shifted = ts0 + _frame_apply_at_caf;
                if (ts0_shifted > -800 && ts0_shifted < 2800) {
                    passCrtSP = true;
                    crt_sp_ts0_in_window.push_back(ts0);
                    nSPInWindow++;
                }
            }
        }

        bool passCrtTrk = false;
        int nTrkInWindow = 0;
        std::vector<double> crt_trk_ts0_in_window;
        if (passFrame){
            for (auto const& ts0: _crt_trk_ts0){
                double ts0_shifted = ts0 + _frame_apply_at_caf;
                if (ts0_shifted > -800 && ts0_shifted < 2800) {
                    passCrtTrk = true;
                    crt_trk_ts0_in_window.push_back(ts0);
                    nTrkInWindow++;
                }
            }
        }
        ...
        if (passFrame && passCrtSP) passFilter = true;
        if (passFrame && passCrtTrk) passFilter = true;
        if (fDebugFilter){
            std::cout << "Event " << _event << " pass frame = " << passFrame 
                    << ", pass CRT SP = " << passCrtSP << " with " << nSPInWindow << " SP in window"
                    << ", pass CRT Trk = " << passCrtTrk << " with " << nTrkInWindow << " Trk in window"
                    << " => pass filter = " << passFilter << std::endl;
            if (passCrtSP){
                std::cout << "  CRT SP ts0 in window: ";
                for (auto const& ts0: crt_sp_ts0_in_window){
                    std::cout << ts0 << " ";
                }
                std::cout << std::endl;
            }
            if (passCrtTrk){
                std::cout << "  CRT Trk ts0 in window: ";
                for (auto const& ts0: crt_trk_ts0_in_window){
                    std::cout << ts0 << " ";
                }
                std::cout << std::endl;
            }
        }
        if (passFilter) fTree->Fill();
    }
    else{
...
void sbnd::BeamAnalysis::beginJob()
{
    ...
    fSubRunTree->Branch("subrun_pot", &fSubRunPOT);
    fSubRunTree->Branch("BNBGates", &fBNBGates);
    fSubRunTree->Branch("OffbeamBNBGates", &fOffbeamBNBGates);
}

void sbnd::BeamAnalysis::endJob()
{
    TH1* hPOT = tfs->make<TH1D>("TotalPOT", "TotalPOT;; POT", 1, 0, 1);
...
void sbnd::BeamAnalysis::beginSubRun(const art::SubRun &sr){

    std::cout << ">>>>> Filling POT for Run " << sr.run() << ": SubRun " << sr.subRun() << std::endl;
    ...
    //Get info
    auto bnb_spill          = sr.getHandle<std::vector<sbn::BNBSpillInfo>>(fBNBPOTDataLabel);
    auto bnb_offbeam_spill  = sr.getHandle<std::vector<sbn::EXTCountInfo>>(fOffbeamBNBCountDataLabel);

    if (bnb_spill && bnb_offbeam_spill) {
        std::cerr << "ERROR: Found both " << fBNBPOTDataLabel << " and " << fOffbeamBNBCountDataLabel << " in the same SubRun. This should not happen. Please check your input file." << std::endl;
        abort();
    }
    if (!bnb_spill && !bnb_offbeam_spill) {
        std::cerr << "ERROR: Found neither " << fBNBPOTDataLabel << " nor " << fOffbeamBNBCountDataLabel << " in this SubRun. This should not happen. Please check your input file." << std::endl;
        abort();
    }

    if(bnb_spill) std::cout << fBNBPOTDataLabel << " ";
    if(bnb_offbeam_spill) std::cout << fOffbeamBNBCountDataLabel << " ";

    if(bnb_spill){
        caf::FillExposure(*bnb_spill, fBNBInfo, fSubRunPOT);
        fTotalPOT += fSubRunPOT;
        fBNBGates += fBNBInfo.size();
        ...
        //    if ( (storedInfo.event == UINT_MAX) || caf::spillInfoToTimestamp(info) > caf::spillInfoToTimestamp(storedInfo) ) {
    ...
    if (bnb_spill) std::cout << "   BNB Spill Info size: " << bnb_spill->size() << std::endl;
    if (bnb_offbeam_spill) std::cout << "   Off-beam BNB Count Info size: " << bnb_offbeam_spill->size() << std::endl;
    std::cout << "   BNB Gates in this SubRun: " << fBNBGates << std::endl;
    std::cout << "   Off-beam BNB Gates in this SubRun: " << fOffbeamBNBGates << std::endl;
    if (fSubRunPOT > 0) std::cout << "   POT: " << fSubRunPOT << std::endl;
}

void sbnd::BeamAnalysis::endSubRun(const art::SubRun& sr) {
    fSubRunTree->Fill();
}
