void BucketStructureT0TDC(const double period = 18.94)
{
  ...
  const double beam_start = 332895.97;
  const double beam_end   = beam_start + 81 * period;
  const double beam_start_rwm = -1048.03;
  const double beam_end_rwm   = beam_start_rwm + 81 * period;
  ...
  std::vector<double> *chit_t0 = 0, *chit_unix_s = 0, *chit_t1 = 0;
  ...
  TH1F *ts0_to_bes_t1_comparison_hist = new TH1F("ts0_to_bes_t1_comparison_hist", ";(CRT t0 -TDC BES) - CRT t1 [ns];CRTHits", 20, -10, 10);
  ...
  TH1F *ts0_to_rwm_t1_comparison_hist = new TH1F("ts0_to_rwm_t1_comparison_hist", ";(CRT t0 -TDC RWM) - CRT t1 [ns];CRTHits", 75,-334000, -333850);
      ...
      for(unsigned ii = 0; ii < chit_t0->size(); ++ii)
        {
          ...
          bool bad_feb = false;
          for(auto mac5 : chit_sipm_feb_mac5->at(ii))
            if(mac5 == 3) bad_feb = true;
          ...
          if(bes_time > beam_start && bes_time < beam_end)
            {
              ...
              ts0_to_bes_t1_comparison_hist->Fill(bes_time - t1);
            }
          ...
          if(rwm_time > beam_start_rwm && rwm_time < beam_end_rwm)
            {
              ...
              ts0_to_rwm_t1_comparison_hist->Fill(rwm_time - t1);
            }
        }
    }
  ...
  ts0_to_bes_bucket_hist->Draw("histE");
  ...
  gausFunc->SetParameters(200,10,4,100);
  gausFunc->SetLineColor(kRed+2);
  gausFunc->SetLineWidth(4);
  TFitResultPtr fit = ts0_to_bes_bucket_hist->Fit("gausFunc", "S", "same");
  gausFunc->Draw("same");
  ...
  TCanvas *ts0_to_bes_t1_comparison_canvas = new TCanvas("ts0_to_bes_t1_comparison_canvas", "ts0_to_bes_t1_comparison_canvas");
  ts0_to_bes_t1_comparison_canvas->cd();
  ts0_to_bes_t1_comparison_canvas->SetRightMargin(.17);
  ts0_to_bes_t1_comparison_canvas->SetLeftMargin(.18);
  ts0_to_bes_t1_comparison_canvas->SetTopMargin(.13);
  ts0_to_bes_t1_comparison_hist->SetLineColor(kMagenta+2);
  ts0_to_bes_t1_comparison_hist->Draw("hist");

  if(save)
    {
      ts0_to_bes_t1_comparison_canvas->SaveAs(saveDir + "/" + run_name + "/ts0_to_bes_t1_comparison.png");
      ts0_to_bes_t1_comparison_canvas->SaveAs(saveDir + "/" + run_name + "/ts0_to_bes_t1_comparison.pdf");
    }
  ...
  ts0_to_rwm_bucket_hist->Draw("histE");
  ...
  TF1 *gausFunc_rwm = new TF1("gausFunc_rwm", "[0]*exp(-0.5*((x-[1])/[2])^2) + [3]", 0, period);
  gausFunc_rwm->SetParameters(200,10,4,40);
  ...
  ptFit_rwm->AddText(Form("Mean:      %.3f", gausFunc_rwm->GetParameter(1)));
  ptFit_rwm->AddText(Form("Sigma:     %.3f", gausFunc_rwm->GetParameter(2)));
  ptFit_rwm->AddText(Form("Bkgd Rate: %.3f", gausFunc_rwm->GetParameter(3)));
  ...
  TCanvas *ts0_to_rwm_t1_comparison_canvas = new TCanvas("ts0_to_rwm_t1_comparison_canvas", "ts0_to_rwm_t1_comparison_canvas");
  ts0_to_rwm_t1_comparison_canvas->cd();
  ts0_to_rwm_t1_comparison_canvas->SetRightMargin(.17);
  ts0_to_rwm_t1_comparison_canvas->SetLeftMargin(.18);
  ts0_to_rwm_t1_comparison_canvas->SetTopMargin(.13);
  ts0_to_rwm_t1_comparison_hist->SetLineColor(kMagenta+2);
  ts0_to_rwm_t1_comparison_hist->Draw("hist");

  if(save)
    {
      ts0_to_rwm_t1_comparison_canvas->SaveAs(saveDir + "/" + run_name + "/ts0_to_rwm_t1_comparison.png");
      ts0_to_rwm_t1_comparison_canvas->SaveAs(saveDir + "/" + run_name + "/ts0_to_rwm_t1_comparison.pdf");
    }
}
