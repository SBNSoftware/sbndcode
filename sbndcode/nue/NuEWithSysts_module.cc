...
private:
  ...
  //std::vector<std::map<std::string, std::vector<float>>> reco_sliceFluxWeights;

  /* 
  std::vector<double>   reco_sliceMCTruthFlux_weight_horncurrent;
  std::vector<double>   reco_sliceMCTruthFlux_weight_expskin;
  std::vector<double>   reco_sliceMCTruthFlux_weight_pioninexsec;
  std::vector<double>   reco_sliceMCTruthFlux_weight_pionqexsec;
  std::vector<double>   reco_sliceMCTruthFlux_weight_piontotxsec;
  std::vector<double>   reco_sliceMCTruthFlux_weight_nucleoninexsec;
  std::vector<double>   reco_sliceMCTruthFlux_weight_nucleonqexsec;
  std::vector<double>   reco_sliceMCTruthFlux_weight_nucleontotxsec;
  std::vector<double>   reco_sliceMCTruthFlux_weight_kplus;
  std::vector<double>   reco_sliceMCTruthFlux_weight_kmin;
  std::vector<double>   reco_sliceMCTruthFlux_weight_kzero;
  std::vector<double>   reco_sliceMCTruthFlux_weight_piplus;
  std::vector<double>   reco_sliceMCTruthFlux_weight_piminus;
  */
  ...
  mcTruthFluxLabel(p.get<std::string>("MCTruthFluxLabel"))
{
  ...
  //NuETree->Branch("reco_sliceFluxWeights", &reco_sliceFluxWeights);
...
void sbnd::NuEWithSysts::beginSubRun(const art::SubRun &sr){
...
void sbnd::NuEWithSysts::endSubRun(const art::SubRun &sr){
...
std::array<double, 4> sbnd::NuEWithSysts::computePCAAngle(const std::vector<art::Ptr<recob::SpacePoint>>& spacePointVec, double vx, double vy, double vz){
...
void sbnd::NuEWithSysts::angleRecalculatePCASlice(const art::Event &e){
...
void sbnd::NuEWithSysts::angleRecalculatePCAPFP(const art::Event &e){
...
int sbnd::NuEWithSysts::GetNumGenEvents(const art::Event &e){
    int nGenEvents = 0;
...
double sbnd::NuEWithSysts::Completeness(const art::Event &e, const std::vector<art::Ptr<recob::Hit>> &objectHits, const int ID){
...
double sbnd::NuEWithSysts::Purity(const art::Event &e, const std::vector<art::Ptr<recob::Hit>> &objectHits, const int ID){
    const detinfo::DetectorClocksData clockData = art::ServiceHandle<detinfo::DetectorClocksService>()->DataFor(e);
...
void sbnd::NuEWithSysts::ClearMaps(const art::Event &e){
    fHitsMap.clear();
}

void sbnd::NuEWithSysts::SetupMaps(const art::Event &e){
    const detinfo::DetectorClocksData clockData = art::ServiceHandle<detinfo::DetectorClocksService>()->DataFor(e);
...
void sbnd::NuEWithSysts::PFPs(art::Event const& e){
...
void sbnd::NuEWithSysts::MCParticles(art::Event const& e){
    art::Handle<std::vector<simb::MCTruth>> MCTruthHandle;
    std::vector<art::Ptr<simb::MCTruth>> MCTruthVec;
    if(e.getByLabel(TruthLabel, MCTruthHandle))
        art::fill_ptr_vector(MCTruthVec, MCTruthHandle);
...
void sbnd::NuEWithSysts::Slices(art::Event const& e){
    ...
    if(counter == 0){
        ...
        //reco_sliceFluxWeights.push_back(-999999);
    }
