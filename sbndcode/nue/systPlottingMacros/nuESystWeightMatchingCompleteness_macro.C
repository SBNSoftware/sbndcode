...
void nuESystWeightMatchingCompleteness_macro(){
    ...
    for(Long64_t e = 0; e < numEntries; ++e){
        ...
        for(size_t slice = 0; slice < reco_sliceID->size(); ++slice){
            ...
            if(sliceCategoryPlottingMacro != -999999){
                ...
                double completeness = reco_sliceCompleteness->at(slice);
                ...
                for(int p = 0; p < nParams; p++){
                    nominal_CRUMBS[cat][p]->Fill(completeness, potWeight);
                }
                ...
                for(int u = 0; u < NUNIV; u++){
                    ...
                    for(int p = 0; p < nParams; p++){
                        univ_CRUMBS[cat][p][u]->Fill(completeness, potWeight * systWeights[p]);
                    }
                }
            }
    ...
    auto plotUniverseDist = [&](const std::string& paramName, TH1D* h, double nominal){
        ...
        std::string outPath = "/nashome/c/coackley/systPlots_completeness/nuE_signalCount_" + paramName + ".pdf";
...
auto plotCRUMBSSyst = [&](int cat, int p){
        ...
        cLeft->Update();
        std::string outPathLeft = "/nashome/c/coackley/systPlots_completeness/completeness_universes_" + catNames[cat] + "_" + paramNames_CRUMBS[p] + ".pdf";
        ...
        TCanvas *cRight = new TCanvas(("c_completeness_sigma_" + catNames[cat] + "_" + paramNames_CRUMBS[p]).c_str(), "", 800, 600);
        ...
        h_mean->GetXaxis()->SetTitle("Completeness");
        ...
        cRight->Update();
        std::string outPathRight = "/nashome/c/coackley/systPlots_completeness/completeness_sigma_" + catNames[cat] + "_" + paramNames_CRUMBS[p] + ".pdf";
