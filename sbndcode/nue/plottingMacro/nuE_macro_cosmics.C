...
void styleDraw(TCanvas* canvas, TH1F* current, TH1F* cheated, TH1F* dune, TH1F* uboone, double ymin, double ymax, double xmin, double xmax, const char* filename, double Lxmin, double Lxmax, double Lymin, double Lymax, TPaveText* pt = nullptr, int* percentage = nullptr, int* drawLine = nullptr){
    ...
    if(drawLine){
        ...
        TLatex* latex = new TLatex(1.022 - 0.2, current->GetMaximum() * 0.93, "2m_{e}");
...
void efficiency(TH1F* current, TH1F* cheated, TH1F* dune, TH1F* uboone, double sizeCurrent, double sizeCheated, double sizeDune, double sizeUboone, double ymin, double ymax, double xmin, double xmax, const char* filename, double Lxmin, double Lxmax, double Lymin, double Lymax, int* drawLine = nullptr, std::string xlabel = ""){
...
recoNeutrino chooseRecoNeutrino(std::vector<recoNeutrino> recoNeutrinoVec, std::vector<recoSlice> recoSliceVec){
    int chosenNeutrinoIndex;
    double highestSliceScore = -100;
    // If there is more than 1 reco Neutrino, choose the reco neutrino in the slice with the highest CRUMBS score

    for(size_t j = 0; j < recoNeutrinoVec.size(); ++j){
        for(size_t i = 0; i < recoSliceVec.size(); ++i){
            if(recoSliceVec[i].id == recoNeutrinoVec[j].sliceID && recoSliceVec[i].score > highestSliceScore){
                //printf("Slice associated with reco neutrino has score = %f\n", recoSliceVec[i].score);
                highestSliceScore = recoSliceVec[i].score;
                chosenNeutrinoIndex = j;
            }
        }
    }

    return recoNeutrinoVec[chosenNeutrinoIndex];
}

recoSlice chooseRecoSlice(std::vector<recoSlice> recoSliceVec, double recoNeutrinoSliceID){
    int chosenSliceIndex;

    // Choosing slice with the highest CRUMBS score
    //double highestScore = -100;
    //for(size_t j = 0; j < recoSliceVec.size(); ++j){
        //if(recoSliceVec[j].score > highestScore){
            //highestScore = recoSliceVec[j].score;
            //chosenSliceIndex = j;
        //}
    //}

    // Choosing slice associated with the reco neutrino
    //printf("Reco Neutrino in Slice ID: %f\n", recoNeutrinoSliceID);
    for(size_t j = 0; j < recoSliceVec.size(); ++j){
        if(recoSliceVec[j].id == recoNeutrinoSliceID) chosenSliceIndex = j;
    }   

    //printf("Chosen Slice Index: %d, Chosen Slice ID: %f\n", chosenSliceIndex, recoSliceVec[chosenSliceIndex].id);
    return recoSliceVec[chosenSliceIndex];
}

recoParticle chooseRecoParticle(std::vector<recoParticle> recoParticleVec, double& totalEnergy, double& chosenSliceID){
   totalEnergy = 0; 
   int chosenParticleIndex = 0;
   double highestEnergy = -10000;

    for(size_t j = 0; j < recoParticleVec.size(); ++j){
        if(recoParticleVec[j].sliceID == chosenSliceID){
...
trueParticle chooseTrueParticle(std::vector<trueParticle> trueParticleVec, double trueVX, double trueVY, double trueVZ){
    int chosenParticleIndex = 0;
    
    for(size_t j = 0; j < trueParticleVec.size(); ++j){
        if(trueParticleVec[j].pdg == 11 && trueParticleVec[j].neutrinoParent == 1 && trueParticleVec[j].vx == trueVX && trueParticleVec[j].vy == trueVY && trueParticleVec[j].vz == trueVZ){
            chosenParticleIndex = j;
        }
    }

    //printf("chosen Particle Index: %i\n", chosenParticleIndex);
    //printf("Particle %zu: PDG = %f, Neutrino Parent = %f, Vertex = (%f, %f, %f)\n", chosenParticleIndex, trueParticleVec[chosenParticleIndex].pdg, trueParticleVec[chosenParticleIndex].neutrinoParent, trueParticleVec[chosenParticleIndex].vx, trueParticleVec[chosenParticleIndex].vy, trueParticleVec[chosenParticleIndex].vz);
    return trueParticleVec[chosenParticleIndex];
}

void numberPlots(std::vector<allEventData> allEventsData){
 
    auto numSlices = createHistGroup("numSlices", "Number of Slices in an Event", "Number of Slices", 50, 0, 50);
    auto numPFPs = createHistGroup("numPFPs", "Number of PFPs in an Event", "Number of PFPs", 200, 0, 200);
    auto numPFPs1Slice = createHistGroup("numPFPs1Slice", "Number of PFPs in 1-Slice Events", "Number of PFPs", 50, 0, 50);
    auto numPFPsSlices = createHistGroup("numPFPsSlices", "Number of PFPs in Multi-Slice Events", "Number of PFPs", 200, 0, 200);
    auto numPFPsChosenSlice = createHistGroup("numPFPsChosenSlice", "Number of PFPs in Chosen Slice", "Number of PFPs", 200, 0, 200);
    ...
    for(const auto& event : allEventsData){
        if(!(event.eventBeforeCut.trueNeutrinoVec[0].pdg == -999999 || event.eventBeforeCut.trueNeutrinoVec[0].tpcValid == 0)){
            double chosenSliceID = event.eventAfterCut.chosenSlice.id;
            ...
            int recoParticleCountChosenSlice = 0;
            ...
            if(!(event.eventBeforeCut.recoParticleVec.size() == 1 && event.eventBeforeCut.recoParticleVec[0].pdg == -999999)){
                for(size_t j = 0; j < recoParticleCount; ++j){
                    if(event.eventBeforeCut.recoParticleVec[j].sliceID == chosenSliceID){
                        recoParticleCountChosenSlice++;
                    }
                }
            }

            printf("Number of PFPs in Chosen Slice: %i\n", recoParticleCountChosenSlice);

            if(event.eventBeforeCut.DLCurrent == 0){
                sizeDLUboone++;
                numPFPs.uboone->Fill(recoParticleCount);
                numSlices.uboone->Fill(recoSliceCount);
                numPFPsChosenSlice.uboone->Fill(recoParticleCountChosenSlice);
                if(recoSliceCount == 1){
                ...
                numPFPsChosenSlice.dune->Fill(recoParticleCountChosenSlice);
                if(recoSliceCount == 1){
                ...
                numPFPsChosenSlice.current->Fill(recoParticleCountChosenSlice);
                if(recoSliceCount == 1){
                ...
                numPFPsChosenSlice.cheated->Fill(recoParticleCountChosenSlice);
                if(recoSliceCount == 1){
    ...
    styleDraw(numSlices.canvas, numSlices.current, numSlices.cheated, numSlices.dune, numSlices.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numSlices_dist.pdf", 0.56, 0.88, 0.74, 0.86);
    styleDraw(numPFPs.canvas, numPFPs.current, numPFPs.cheated, numPFPs.dune, numPFPs.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numPFPs_dist.pdf", 0.56, 0.88, 0.74, 0.86);
    styleDraw(numPFPs1Slice.canvas, numPFPs1Slice.current, numPFPs1Slice.cheated, numPFPs1Slice.dune, numPFPs1Slice.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numPFPs1Slice_dist.pdf", 0.56, 0.88, 0.70, 0.86);
    styleDraw(numPFPsSlices.canvas, numPFPsSlices.current, numPFPsSlices.cheated, numPFPsSlices.dune, numPFPsSlices.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numPFPsSlices_dist.pdf", 0.56, 0.88, 0.70, 0.86);
    styleDraw(numPFPsChosenSlice.canvas, numPFPsChosenSlice.current, numPFPsChosenSlice.cheated, numPFPsChosenSlice.dune, numPFPsChosenSlice.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numPFPsChosenSlice_dist.pdf", 0.56, 0.88, 0.70, 0.86);

    percentage(numSlices.current, numSlices.cheated, numSlices.dune, numSlices.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numSlices_perc.pdf", 0.56, 0.88, 0.70, 0.86);
    percentage(numPFPs.current, numPFPs.cheated, numPFPs.dune, numPFPs.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numPFPs_perc.pdf", 0.56, 0.88, 0.70, 0.86);
    percentage(numPFPs1Slice.current, numPFPs1Slice.cheated, numPFPs1Slice.dune, numPFPs1Slice.uboone, sizeCurrent1Slice, sizeCheated1Slice, sizeDLDune1Slice, sizeDLUboone1Slice, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numPFPs1Slice_perc.pdf", 0.56, 0.88, 0.70, 0.86);
    percentage(numPFPsSlices.current, numPFPsSlices.cheated, numPFPsSlices.dune, numPFPsSlices.uboone, sizeCurrentSlices, sizeCheatedSlices, sizeDLDuneSlices, sizeDLUbooneSlices, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numPFPsSlices_perc.pdf", 0.56, 0.88, 0.70, 0.86);
    percentage(numPFPsChosenSlice.current, numPFPsChosenSlice.cheated, numPFPsChosenSlice.dune, numPFPsChosenSlice.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/numPFPsSlices_perc.pdf", 0.56, 0.88, 0.70, 0.86);
...
void chosenSlicePlots(std::vector<allEventData> allEvents){
    auto sliceCompleteness = createHistGroup("sliceCompleteness", "Completeness of the Chosen Slice in an Event", "Slice Completeness", 204, 0, 1.02);
    auto sliceCompleteness1Slice = createHistGroup("sliceCompleteness1Slice", "Completeness of Slice in an Event with 1 Slice", "Slice Completeness", 204, 0, 1.02);
    auto sliceCompletenessSlices = createHistGroup("sliceCompletenessSlices", "Completeness of the Chosen Slice in an Event with > 1 Slice", "Slice Completeness", 204, 0, 1.02);
    auto sliceScore = createHistGroup("sliceScore", "Score of the Chosen Slice in an Event", "Slice Score", 50, -1, 1);
    ...
    styleDraw(sliceCompleteness.canvas, sliceCompleteness.current, sliceCompleteness.cheated, sliceCompleteness.dune, sliceCompleteness.uboone, 0, 1000, 0, 1.005, "/nashome/c/coackley/nuEPlotsWithCosmics/chosenSliceCompleteness_dist_zoomed.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    styleDraw(sliceCompleteness1Slice.canvas, sliceCompleteness1Slice.current, sliceCompleteness1Slice.cheated, sliceCompleteness1Slice.dune, sliceCompleteness1Slice.uboone, 0, 1000, 0, 1.005, "/nashome/c/coackley/nuEPlotsWithCosmics/chosenSliceCompleteness1Slice_dist_zoomed.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    styleDraw(sliceCompletenessSlices.canvas, sliceCompletenessSlices.current, sliceCompletenessSlices.cheated, sliceCompletenessSlices.dune, sliceCompletenessSlices.uboone, 0, 1000, 0, 1.05, "/nashome/c/coackley/nuEPlotsWithCosmics/chosenSliceCompletenessSlices_dist.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    styleDraw(sliceScore.canvas, sliceScore.current, sliceScore.cheated, sliceScore.dune, sliceScore.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/chosenSliceScore_dist_bigBins.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    
    percentage(sliceCompleteness.current, sliceCompleteness.cheated, sliceCompleteness.dune, sliceCompleteness.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 0, 1.005, "/nashome/c/coackley/nuEPlotsWithCosmics/chosenSliceCompleteness_perc_zoomed.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    percentage(sliceCompleteness1Slice.current, sliceCompleteness1Slice.cheated, sliceCompleteness1Slice.dune, sliceCompleteness1Slice.uboone, sizeCurrent1Slice, sizeCheated1Slice, sizeDLDune1Slice, sizeDLUboone1Slice, 0, 100, 0, 1.005, "/nashome/c/coackley/nuEPlotsWithCosmics/chosenSliceCompleteness1Slice_perc_zoomed.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    percentage(sliceCompletenessSlices.current, sliceCompletenessSlices.cheated, sliceCompletenessSlices.dune, sliceCompletenessSlices.uboone, sizeCurrent - sizeCurrent1Slice, sizeCheated - sizeCheated1Slice, sizeDLDune - sizeDLDune1Slice, sizeDLUboone - sizeDLUboone1Slice, 0, 100, 0, 1.05, "/nashome/c/coackley/nuEPlotsWithCosmics/chosenSliceCompletenessSlices_perc.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    percentage(sliceScore.current, sliceScore.cheated, sliceScore.dune, sliceScore.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/chosenSliceScore_perc_bigBins.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
}

void deltaVertex(std::vector<allEventData> allEventsData){
    auto deltaX = createHistGroup("deltaX", "#Deltax Distribution: Nu + E Elastic Scattering Events", "x_{Reco} - x_{True} (cm)", 160, -20, 20);
    auto deltaY = createHistGroup("deltaY", "#Deltay Distribution: Nu + E Elastic Scattering Events", "y_{Reco} - y_{True} (cm)", 160, -20, 20);
    auto deltaZ = createHistGroup("deltaZ", "#Deltaz Distribution: Nu + E Elastic Scattering Events", "z_{Reco} - z_{True} (cm)", 160, -20, 20);
    auto deltaR = createHistGroup("deltaR", "#Delta#bar{r} Distribution: Nu + E Elastic Scattering Events", "|#bar{r}_{Reco} - #bar{r}_{True}| (cm)", 80, 0, 20);
    ...
    styleDraw(deltaX.canvas, deltaX.current, deltaX.cheated, deltaX.dune, deltaX.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaX_dist.pdf", 0.55, 0.88, 0.77, 0.86);
    styleDraw(deltaY.canvas, deltaY.current, deltaY.cheated, deltaY.dune, deltaY.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaY_dist.pdf", 0.55, 0.88, 0.77, 0.86);
    styleDraw(deltaZ.canvas, deltaZ.current, deltaZ.cheated, deltaZ.dune, deltaZ.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaZ_dist.pdf", 0.55, 0.88, 0.77, 0.86);
    styleDraw(deltaR.canvas, deltaR.current, deltaR.cheated, deltaR.dune, deltaR.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaR_dist.pdf", 0.55, 0.88, 0.77, 0.86);
              
    percentage(deltaX.current, deltaX.cheated, deltaX.dune, deltaX.uboone, sizeCurrent, sizeCheated, sizeDune, sizeUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaX_perc.pdf", 0.55, 0.88, 0.78, 0.86);
    percentage(deltaY.current, deltaY.cheated, deltaY.dune, deltaY.uboone, sizeCurrent, sizeCheated, sizeDune, sizeUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaY_perc.pdf", 0.55, 0.88, 0.78, 0.86);
    percentage(deltaZ.current, deltaZ.cheated, deltaZ.dune, deltaZ.uboone, sizeCurrent, sizeCheated, sizeDune, sizeUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaZ_perc.pdf", 0.55, 0.88, 0.78, 0.86);
    percentage(deltaR.current, deltaR.cheated, deltaR.dune, deltaR.uboone, sizeCurrent, sizeCheated, sizeDune, sizeUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaR_perc.pdf", 0.55, 0.88, 0.78, 0.86);
}
...
void energyAngle(std::vector<allEventData> allEventsData){
    ...
    auto deltaEChosenShower = createHistGroup("deltaE Chosen Shower", "#DeltaE Distribution for E_{reco} Being Chosen Shower Energy", "E_{reco} - E_{true} (MeV)", 35, -600, 100);
    ...
    styleDraw(deltaEAllEnergy.canvas, deltaEAllEnergy.current, deltaEAllEnergy.cheated, deltaEAllEnergy.dune, deltaEAllEnergy.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaEAllEnergy_dist.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    styleDraw(EDividedAllEnergy.canvas, EDividedAllEnergy.current, EDividedAllEnergy.cheated, EDividedAllEnergy.dune, EDividedAllEnergy.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/EDividedAllEnergy_dist.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    styleDraw(deltaEChosenShower.canvas, deltaEChosenShower.current, deltaEChosenShower.cheated, deltaEChosenShower.dune, deltaEChosenShower.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaEChosenShower_dist.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    styleDraw(EDividedChosenShower.canvas, EDividedChosenShower.current, EDividedChosenShower.cheated, EDividedChosenShower.dune, EDividedChosenShower.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/EDividedChosenShower_dist.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    styleDraw(EChosenShowerEAllEnergyDivided.canvas, EChosenShowerEAllEnergyDivided.current, EChosenShowerEAllEnergyDivided.cheated, EChosenShowerEAllEnergyDivided.dune, EChosenShowerEAllEnergyDivided.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/EChosenShowerEAllEnergyDivided_dist.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    styleDraw(ETrueThetaTrue2.canvas, ETrueThetaTrue2.current, ETrueThetaTrue2.cheated, ETrueThetaTrue2.dune, ETrueThetaTrue2.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaTrue2_dist.pdf", 0.56, 0.88, 0.70, 0.86, nullptr, nullptr, &drawLine);
    styleDraw(ETrueThetaReco2.canvas, ETrueThetaReco2.current, ETrueThetaReco2.cheated, ETrueThetaReco2.dune, ETrueThetaReco2.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaReco2_dist.pdf", 0.56, 0.88, 0.70, 0.86, nullptr, nullptr, &drawLine);
    styleDraw(ERecoThetaTrue2AllEnergy.canvas, ERecoThetaTrue2AllEnergy.current, ERecoThetaTrue2AllEnergy.cheated, ERecoThetaTrue2AllEnergy.dune, ERecoThetaTrue2AllEnergy.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaTrue2AllEnergy_dist.pdf", 0.56, 0.88, 0.70, 0.86, nullptr, nullptr, &drawLine);
    styleDraw(ERecoThetaTrue2ChosenShower.canvas, ERecoThetaTrue2ChosenShower.current, ERecoThetaTrue2ChosenShower.cheated, ERecoThetaTrue2ChosenShower.dune, ERecoThetaTrue2ChosenShower.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaTrue2ChosenShower_dist.pdf", 0.56, 0.88, 0.70, 0.86, nullptr, nullptr, &drawLine);
    styleDraw(ERecoThetaReco2AllEnergy.canvas, ERecoThetaReco2AllEnergy.current, ERecoThetaReco2AllEnergy.cheated, ERecoThetaReco2AllEnergy.dune, ERecoThetaReco2AllEnergy.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaReco2AllEnergy_dist.pdf", 0.56, 0.88, 0.70, 0.86, nullptr, nullptr, &drawLine);
    styleDraw(ERecoThetaReco2ChosenShower.canvas, ERecoThetaReco2ChosenShower.current, ERecoThetaReco2ChosenShower.cheated, ERecoThetaReco2ChosenShower.dune, ERecoThetaReco2ChosenShower.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaReco2ChosenShower_dist.pdf", 0.56, 0.88, 0.70, 0.86, nullptr, nullptr, &drawLine);
    styleDraw(angleBetweenDirections.canvas, angleBetweenDirections.current, angleBetweenDirections.cheated, angleBetweenDirections.dune, angleBetweenDirections.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/angleDiffDirections_dist.pdf", 0.56, 0.88, 0.70, 0.86);
    styleDraw(ETrueThetaTrue2BeforeCuts.canvas, ETrueThetaTrue2BeforeCuts.current, ETrueThetaTrue2BeforeCuts.cheated, ETrueThetaTrue2BeforeCuts.dune, ETrueThetaTrue2BeforeCuts.uboone, 0, 1000, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaTrue2BeforeCuts_dist.pdf", 0.56, 0.88, 0.70, 0.86, nullptr, nullptr, &drawLine);

    percentage(deltaEAllEnergy.current, deltaEAllEnergy.cheated, deltaEAllEnergy.dune, deltaEAllEnergy.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaEAllEnergy_perc.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    percentage(EDividedAllEnergy.current, EDividedAllEnergy.cheated, EDividedAllEnergy.dune, EDividedAllEnergy.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/EDividedAllEnergy_perc.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    percentage(deltaEChosenShower.current, deltaEChosenShower.cheated, deltaEChosenShower.dune, deltaEChosenShower.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/deltaEChosenShower_perc.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    percentage(EDividedChosenShower.current, EDividedChosenShower.cheated, EDividedChosenShower.dune, EDividedChosenShower.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/EDividedChosenShower_perc.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    percentage(EChosenShowerEAllEnergyDivided.current, EChosenShowerEAllEnergyDivided.cheated, EChosenShowerEAllEnergyDivided.dune, EChosenShowerEAllEnergyDivided.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/EChosenShowerEAllEnergyDivided_perc.pdf", 1-0.86, 1-0.54, 0.70, 0.86);
    percentage(ETrueThetaTrue2.current, ETrueThetaTrue2.cheated, ETrueThetaTrue2.dune, ETrueThetaTrue2.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaTrue2_perc.pdf", 0.56, 0.88, 0.70, 0.86, &drawLine);
    percentage(ETrueThetaReco2.current, ETrueThetaReco2.cheated, ETrueThetaReco2.dune, ETrueThetaReco2.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaReco2_perc.pdf", 0.56, 0.88, 0.70, 0.86, &drawLine);
    percentage(ERecoThetaTrue2AllEnergy.current, ERecoThetaTrue2AllEnergy.cheated, ERecoThetaTrue2AllEnergy.dune, ERecoThetaTrue2AllEnergy.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaTrue2AllEnergy_perc.pdf", 0.56, 0.88, 0.70, 0.86, &drawLine);
    percentage(ERecoThetaTrue2ChosenShower.current, ERecoThetaTrue2ChosenShower.cheated, ERecoThetaTrue2ChosenShower.dune, ERecoThetaTrue2ChosenShower.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaTrue2ChosenShower_perc.pdf", 0.56, 0.88, 0.70, 0.86, &drawLine);
    percentage(ERecoThetaReco2AllEnergy.current, ERecoThetaReco2AllEnergy.cheated, ERecoThetaReco2AllEnergy.dune, ERecoThetaReco2AllEnergy.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaReco2AllEnergy_perc.pdf", 0.56, 0.88, 0.70, 0.86, &drawLine);
    percentage(ERecoThetaReco2ChosenShower.current, ERecoThetaReco2ChosenShower.cheated, ERecoThetaReco2ChosenShower.dune, ERecoThetaReco2ChosenShower.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaReco2ChosenShower_perc.pdf", 0.56, 0.88, 0.70, 0.86, &drawLine);
    percentage(angleBetweenDirections.current, angleBetweenDirections.cheated, angleBetweenDirections.dune, angleBetweenDirections.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/angleDiffDirections_perc.pdf", 0.56, 0.88, 0.70, 0.86);
    percentage(ETrueThetaTrue2BeforeCuts.current, ETrueThetaTrue2BeforeCuts.cheated, ETrueThetaTrue2BeforeCuts.dune, ETrueThetaTrue2BeforeCuts.uboone, sizeCurrentBefore, sizeCheatedBefore, sizeDLDuneBefore, sizeDLUbooneBefore, 0, 100, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaTrue2BeforeCuts_perc.pdf", 0.56, 0.88, 0.70, 0.86, &drawLine);

    efficiency(ETrueThetaTrue2.current, ETrueThetaTrue2.cheated, ETrueThetaTrue2.dune, ETrueThetaTrue2.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 1, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaTrue2_eff.pdf", 0.56, 0.88, 0.14, 0.3, &drawLine, "E_{true}#theta_{true}^{2} (MeV) Cut");
    efficiency(ETrueThetaReco2.current, ETrueThetaReco2.cheated, ETrueThetaReco2.dune, ETrueThetaReco2.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 1, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaReco2_eff.pdf", 0.56, 0.88, 0.14, 0.3, &drawLine, "E_{true}#theta_{reco}^{2} (MeV) Cut");
    efficiency(ERecoThetaTrue2AllEnergy.current, ERecoThetaTrue2AllEnergy.cheated, ERecoThetaTrue2AllEnergy.dune, ERecoThetaTrue2AllEnergy.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 1, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaTrue2AllEnergy_eff.pdf", 0.56, 0.88, 0.14, 0.3, &drawLine, "E_{reco}#theta_{true}^{2} (MeV) Cut"); 
    efficiency(ERecoThetaTrue2ChosenShower.current, ERecoThetaTrue2ChosenShower.cheated, ERecoThetaTrue2ChosenShower.dune, ERecoThetaTrue2ChosenShower.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 1, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaTrue2ChosenShower_eff.pdf", 0.56, 0.88, 0.14, 0.3, &drawLine, "E_{reco}#theta_{true}^{2} (MeV) Cut");
    efficiency(ERecoThetaReco2AllEnergy.current, ERecoThetaReco2AllEnergy.cheated, ERecoThetaReco2AllEnergy.dune, ERecoThetaReco2AllEnergy.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 1, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaReco2AllEnergy_eff.pdf", 0.56, 0.88, 0.14, 0.3, &drawLine, "E_{reco}#theta_{reco}^{2} (MeV) Cut");
    efficiency(ERecoThetaReco2ChosenShower.current, ERecoThetaReco2ChosenShower.cheated, ERecoThetaReco2ChosenShower.dune, ERecoThetaReco2ChosenShower.uboone, sizeCurrent, sizeCheated, sizeDLDune, sizeDLUboone, 0, 1, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ERecoThetaReco2ChosenShower_eff.pdf", 0.56, 0.88, 0.14, 0.3, &drawLine, "E_{reco}#theta_{reco}^{2} (MeV) Cut");
    efficiency(ETrueThetaTrue2BeforeCuts.current, ETrueThetaTrue2BeforeCuts.cheated, ETrueThetaTrue2BeforeCuts.dune, ETrueThetaTrue2BeforeCuts.uboone, sizeCurrentBefore, sizeCheatedBefore, sizeDLDuneBefore, sizeDLUbooneBefore, 0, 1, 999, 999, "/nashome/c/coackley/nuEPlotsWithCosmics/ETrueThetaTrue2BeforeCuts_eff.pdf", 0.56, 0.88, 0.14, 0.3, &drawLine, "E_{true}#theta_{true}^{2} (MeV) Cut");
...
void nuE_macro(){
    ...
    TFile *file = TFile::Open("/exp/sbnd/data/users/coackley/Nu+E_Cosmics/analysed_DL_Uboone/CRUMBS/1.root");
    ...
    for(Long64_t i = 0; i < numEntries; ++i){
        ...
        for(size_t j = 0; j < truth_particlePDG->size(); ++j){
            ...
            particle.neutrinoParent = truth_particleNeutrinoParent->at(j);
        ...
        if(recoNeutrinos.size() == 1){
            ...
            //printf("There is more than one reco neutrino in the event");
            eventCut.chosenRecoNeutrino = chooseRecoNeutrino(recoNeutrinos, slices);
        }
        
        if(slices.size() == 1){
            eventCut.chosenSlice = slices[0];
        } else{
            eventCut.chosenSlice = chooseRecoSlice(slices, eventCut.chosenRecoNeutrino.sliceID);
        }
        ...
        if(recoParticles.size() == 1){
            ...
            eventCut.chosenRecoParticle = chooseRecoParticle(recoParticles, totalEnergyPFPs, eventCut.chosenSlice.id);
        }
        ...
        if(trueParticles.size() == 1){
            if(trueParticles[0].neutrinoParent == 1 && trueParticles[0].pdg == 11){
                // The true particle is an electron whose parent in the neutrino
                eventCut.chosenTrueParticle = trueParticles[0];
            }
        } else{
            // More than 1 true particle
            eventCut.chosenTrueParticle = chooseTrueParticle(trueParticles, eventCut.chosenTrueNeutrino.vx, eventCut.chosenTrueNeutrino.vy, eventCut.chosenTrueNeutrino.vz);
        }
