////////////////////////////////////////////////////////////////////////
// Class:       NuERecoWeightsIntimeCosmic
// Plugin Type: analyzer (Unknown Unknown)
// File:        NuERecoWeightsIntimeCosmic_module.cc
//
...
class sbnd::NuERecoWeightsIntimeCosmic : public art::EDAnalyzer {
public:
  explicit NuERecoWeightsIntimeCosmic(fhicl::ParameterSet const& p);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  NuERecoWeightsIntimeCosmic(NuERecoWeightsIntimeCosmic const&) = delete;
  NuERecoWeightsIntimeCosmic(NuERecoWeightsIntimeCosmic&&) = delete;
  NuERecoWeightsIntimeCosmic& operator=(NuERecoWeightsIntimeCosmic const&) = delete;
  NuERecoWeightsIntimeCosmic& operator=(NuERecoWeightsIntimeCosmic&&) = delete;
...
private:
  ...
  double nuEScatterTrueVX_weightTree;
  double nuEScatterTrueVY_weightTree;
  double nuEScatterTrueVZ_weightTree;
  ...
  std::vector<double> reco_sliceID_weightTree;
  ...
  TFile *outputFile = TFile::Open("NuERecoWeightsIntimeCosmicAnalyserOutput.root","RECREATE");

};


sbnd::NuERecoWeightsIntimeCosmic::NuERecoWeightsIntimeCosmic(fhicl::ParameterSet const& p)
  : EDAnalyzer{p},
  ...
  signal(p.get<double>("Signal"))
{
  ...
  NuEWeightsTree->Branch("nuEScatterTrueVX_weightTree", &nuEScatterTrueVX_weightTree);
  NuEWeightsTree->Branch("nuEScatterTrueVY_weightTree", &nuEScatterTrueVY_weightTree);
  NuEWeightsTree->Branch("nuEScatterTrueVZ_weightTree", &nuEScatterTrueVZ_weightTree);
  ...
  NuEWeightsTree->Branch("reco_sliceID_weightTree", &reco_sliceID_weightTree);
...
void sbnd::NuERecoWeightsIntimeCosmic::beginSubRun(const art::SubRun &sr){
...
void sbnd::NuERecoWeightsIntimeCosmic::endSubRun(const art::SubRun &sr){
    //printf("POT = %f, Spills = %i, Num Events Generated = %i\n", pot, spills, numGenEvents);
    SubRunTree->Fill();
}

void sbnd::NuERecoWeightsIntimeCosmic::analyze(art::Event const& e)
{
    clearVectors();
    ClearMaps(e);
    SetupMaps(e);
...
std::array<double, 4> sbnd::NuERecoWeightsIntimeCosmic::computePCAAngle(const std::vector<art::Ptr<recob::SpacePoint>>& spacePointVec, double vx, double vy, double vz){
...
void sbnd::NuERecoWeightsIntimeCosmic::angleRecalculatePCASlice(const art::Event &e){
...
void sbnd::NuERecoWeightsIntimeCosmic::angleRecalculatePCAPFP(const art::Event &e){
...
int sbnd::NuERecoWeightsIntimeCosmic::GetNumGenEvents(const art::Event &e){
    int nGenEvents = 0;
...
double sbnd::NuERecoWeightsIntimeCosmic::Completeness(const art::Event &e, const std::vector<art::Ptr<recob::Hit>> &objectHits, const int ID){
...
double sbnd::NuERecoWeightsIntimeCosmic::Purity(const art::Event &e, const std::vector<art::Ptr<recob::Hit>> &objectHits, const int ID){
    const detinfo::DetectorClocksData clockData = art::ServiceHandle<detinfo::DetectorClocksService>()->DataFor(e);
...
void sbnd::NuERecoWeightsIntimeCosmic::ClearMaps(const art::Event &e){
    fHitsMap.clear();
}

void sbnd::NuERecoWeightsIntimeCosmic::SetupMaps(const art::Event &e){
    const detinfo::DetectorClocksData clockData = art::ServiceHandle<detinfo::DetectorClocksService>()->DataFor(e);
...
void sbnd::NuERecoWeightsIntimeCosmic::PFPs(art::Event const& e){
...
void sbnd::NuERecoWeightsIntimeCosmic::MCParticles(art::Event const& e){
    art::Handle<std::vector<simb::MCTruth>> MCTruthHandle;
    std::vector<art::Ptr<simb::MCTruth>> MCTruthVec;
    if(e.getByLabel(TruthLabel, MCTruthHandle))
        art::fill_ptr_vector(MCTruthVec, MCTruthHandle);
...
void sbnd::NuERecoWeightsIntimeCosmic::Slices(art::Event const& e){
    const detinfo::DetectorClocksData clockData = art::ServiceHandle<detinfo::DetectorClocksService>()->DataFor(e);
    ...
    if(!sliceVec.empty()){
        ...
        if(!hitVec.empty()){
            ...
            for(const art::Ptr<recob::Slice> &slice : sliceVec){
                ...
                reco_sliceID_weightTree.push_back(sliceID);
...
void sbnd::NuERecoWeightsIntimeCosmic::clearVectors(){
...
void sbnd::NuERecoWeightsIntimeCosmic::beginJob()
{
  // Implementation of optional member function here.
}

void sbnd::NuERecoWeightsIntimeCosmic::endJob()
{
  // Implementation of optional member function here.
}

DEFINE_ART_MODULE(sbnd::NuERecoWeightsIntimeCosmic)
