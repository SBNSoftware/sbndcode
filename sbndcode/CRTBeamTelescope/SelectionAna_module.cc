...
private:
  ...
  int _isSquare;                         ///< Can CRT hits form a rectangle?
  double _RectScore;                     ///< The Score to show how likely the CRT hits can form a rectangle.
  double _biggest_distance_between_hits; ///< Mean of the distance between diagonal hits.
  double _square_side_length;            ///< Side length of the Rectangle.
  double _square_side_width;             ///< Side width of the Rectangle.
  double _square_area;                   ///< Area of the Rectangle.
  ...
  double _chit_depositedE_1st_hit; ///< PE of the first hit.
  double _chit_depositedE_2nd_hit; ///< PE of the second hit.
...
// More initializers here.
{
  ...
  _save_systs_geant4 = p.get<bool>("SaveSystsGeant4", false);
  ...
  _tree->Branch("RectScore", &_RectScore, "RectScore/D");
  ...
  // new variables for the deposited energy.
  _tree->Branch("chit_depositedE_1st_hit", &_chit_depositedE_1st_hit, "chit_depositedE_1st_hit/D");
  _tree->Branch("chit_depositedE_2nd_hit", &_chit_depositedE_2nd_hit, "chit_depositedE_2nd_hit/D");
  ...
  if (!_data_mode)
  {
    ...
    {
      _tree->Branch("chit_backtrack_evtwgt_geant4_weight", &_chit_backtrack_evtwgt_geant4_weight);
    }
  }
...
void SelectionAna::analyze(art::Event const &e)
{
  ...
  _chit_backtrack_purity.clear();
  _chit_backtrack_trackID.clear();
  _chit_backtrack_origin.clear();
  _chit_backtrack_nu_pdg.clear();
  _chit_backtrack_nu_ccnc.clear();
  _chit_backtrack_nu_mode.clear();
  _chit_backtrack_nu_interaction_type.clear();
  ...
  std::vector<double> calibration_constant;
  calibration_constant.resize(2);                   /*calibration_constant[0], upstream, calibration_constant[1] downstream*/
  ...
  if (_cosmic_mode)
    beam_time_window[0] = 1.7e6 - 3000.; // shift the time in the cosmic window.
  ...
  chit_x_downstream.clear();
  chit_y_downstream.clear();
  chit_t1_downstream.clear();
  chit_depoistedE_downstream.clear();
  chit_corrADC_strip_diff_downstream.clear();
  chit_t1_strip_diff_downstream.clear();

  // set the number of hits in the upstream and downstream to 0.
  _n_chits_upstream = 0;
  _n_chits_downstream = 0;
  ...
  for (size_t ihit = 0; ihit < crt_hit_v.size(); ihit++)
  {
    ...
    if (_smear_timing)
    {
      ...
      if (_debug)
        std::cout << "Before hit ts1: " << hit->ts1_ns << ", smear_timing_offset: " << smear_timing_offset << ", frac diff: " << (smear_timing_offset) / hit->ts1_ns << std::endl;
    }
    ...
    if (_debug)
      std::cout << "hit: " << ihit << ", t1: " << t1 << ", beam_time_window: " << beam_time_window[0] << ", " << beam_time_window[1] << std::endl;
    ...
    { // downstream
      ...
      if (_smear_deposited_energy)
      {
        ...
        if (_debug)
          std::cout << "Before hit depositedE: " << hit->peshit * calibration_constant[1] << ", smear_deposited_energy_offset: " << smear_deposited_energy_offset << ", frac diff: " << (smear_deposited_energy_offset) / hit->peshit * calibration_constant[1] << ", sigma_depositedE: " << sigma_depositedE << std::endl;
      }
      else
      {
      ...
      chit_x_downstream.push_back(hit->x_pos);
      chit_y_downstream.push_back(hit->y_pos);
    ...
    { // upstream
      ...
      if (_smear_deposited_energy)
        ...
        _chit_depositedE.push_back(hit->peshit * calibration_constant[0]);
    }
  ...
  std::map<double, std::vector<double>> distance_to_t1_diff_map;
  distance_to_t1_diff_map.clear();
  std::map<double, std::vector<std::vector<double>>> distance_to_chit_depositedE_mean_map;
  distance_to_chit_depositedE_mean_map.clear();
  std::map<double, std::vector<double>> distance_to_chit_depositedE_diff_map;
  distance_to_chit_depositedE_diff_map.clear();
  std::map<double, std::vector<double>> distance_to_chit_depositedE_ratio_map;
  distance_to_chit_depositedE_ratio_map.clear();
  std::map<double, std::vector<double>> distance_to_chit_depositedE_frac_diff_map;
  distance_to_chit_depositedE_frac_diff_map.clear();
  std::map<double, std::vector<std::vector<double>>> distance_to_chit_depositedE_map;
  distance_to_chit_depositedE_map.clear();
  std::vector<std::array<double, 2>> position_x_y;
  position_x_y.clear();
  std::vector<double> distance_between_hits_downstream;
  distance_between_hits_downstream.clear();
  std::map<double, double> distance_to_smeared_distance_map;
  distance_to_smeared_distance_map.clear();
  ...
  if (chit_x_downstream.size() >= 2)
  {
    for (size_t i = 0; i < chit_x_downstream.size(); i++)
    {
      for (size_t j = i + 1; j < chit_x_downstream.size(); j++)
      {
        ...
        if (chit_depoistedE_downstream[i] < chit_depoistedE_downstream[j])
          chit_depositedE_ratio_between_hits = std::abs(chit_depoistedE_downstream[i] / chit_depoistedE_downstream[j]);
        else
          chit_depositedE_ratio_between_hits = std::abs(chit_depoistedE_downstream[j] / chit_depoistedE_downstream[i]);
        ...
        distance_to_chit_depositedE_map[distance].push_back({chit_depoistedE_downstream[i], chit_depoistedE_downstream[j]});
      }
  ...
  _RectScore = FormRectScore(distance_between_hits_downstream);
  if (canFormRect(distance_between_hits_downstream))
  {
    _isSquare = 1; // true
  ...
  _chit_depositedE_1st_hit = -1.;
  _chit_depositedE_2nd_hit = -1.;
  ...
  {
    if (counter == distance_to_chit_depositedE_mean_map.size() - 1)
    { // the maximum distance should belongs to the diagonal hits.
      // read in the maps.
      ...
      auto chit_depositedE_vec = distance_to_chit_depositedE_map[distance];

      if (chit_depositedE_mean_vec.size() == 2)
      { // two diagonal pairs.
        ...
        if (_debug)
          std::cout << "Before distance: " << distance << ", smear_position_offset: " << smear_position_offset << ", frac diff: " << smear_position_offset / distance << std::endl;
        ...
        if (chit_depositedE_mean_vec[1][1] < t1_strip_diff_min)
        {
          index = 1;
        }
        else if (chit_depositedE_mean_vec[1][1] == t1_strip_diff_min)
        {
          if (chit_depositedE_mean_vec[1][2] < chit_depositedE_mean_vec[0][2])
          {
            index = 1;
          }
        }
        ...
        _chit_depositedE_1st_hit = chit_depositedE_vec[index][0];
        _chit_depositedE_2nd_hit = chit_depositedE_vec[index][1];
      }
    }
    counter++;
  }
...
double SelectionAna::FormRectScore(std::vector<double> distances)
{
  if (distances.size() != 6)
  {
    return -1.; // Need exactly 4 points to form a square
  }

  // Sort the distances in non-decreasing order
  std::sort(distances.begin(), distances.end());

  double side_diff = std::abs(distances[0] - distances[1]);
  double side_diff_2 = std::abs(distances[2] - distances[3]);
  double diagonal_diff = std::abs(distances[4] - distances[5]);
  double Pythagorean_diff = std::abs(distances[4] - std::sqrt(distances[0] * distances[0] + distances[2] * distances[2]));
...
void SelectionAna::StoreNeutrinoInfo(art::Event const &e, std::string _geant_producer, int geant_track_id, int _interactionMode, int &nu_pdg, int &ccnc, int &nu_mode, int &nu_interaction_type, double &nu_energy, int &nu_source)
{
  ...
  for (auto iter : particlesToTruth)
  {
    if (iter.first->TrackId() == geant_track_id)
    {
      ...
      { // check if the neutrino is from the beam.
