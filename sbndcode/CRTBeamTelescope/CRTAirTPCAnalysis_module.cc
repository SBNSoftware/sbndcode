////////////////////////////////////////////////////////////////////////
// Class:       CRTAirTPCAnalysis
// Plugin Type: analyzer (Unknown Unknown)
// File:        CRTAirTPCAnalysis_module.cc
//
...
class CRTAirTPCAnalysis : public art::EDAnalyzer {
public:
  explicit CRTAirTPCAnalysis(fhicl::ParameterSet const& p);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  CRTAirTPCAnalysis(CRTAirTPCAnalysis const&) = delete;
  CRTAirTPCAnalysis(CRTAirTPCAnalysis&&) = delete;
  CRTAirTPCAnalysis& operator=(CRTAirTPCAnalysis const&) = delete;
  CRTAirTPCAnalysis& operator=(CRTAirTPCAnalysis&&) = delete;
...
private:
  ...
  std::vector<float> _nu_e; ///< Neutrino energy
  std::vector<int> _nu_pdg; ///< Neutrino PDG code
  std::vector<int> _nu_ccnc; ///< 0: CC, 1: NC
  std::vector<int> _nu_mode; ///< Neutrino interaction mode
  std::vector<int> _nu_int_type; ///< Neutrino interaction type
  std::vector<float> _nu_vtx_x; ///< Neutrino vertex X
  std::vector<float> _nu_vtx_y; ///< Neutrino vertex Y
  std::vector<float> _nu_vtx_z; ///< Neutrino vertex Z
  std::vector<float> _nu_px; ///< Neutrino momentum along X
  std::vector<float> _nu_py; ///< Neutrino momentum along Y
  std::vector<float> _nu_pz; ///< Neutrino momentum along Z
  ...
  std::vector<double> _mcp_t;    ///< G4 MCParticle time
  ...
  std::vector<int>    _mcp_origin;   ///< G4 MCParticle, 0 -- signal; 1 -- dirt; 2 -- cosmic;

  // geant 4
  std::vector<double> _adh_t;          ///< AuxDetHit time
  std::vector<double> _adh_t_diff;     ///< AuxDetHit time difference between two hits
  ...
  std::vector<double> _adh_entryX;     ///< AuxDetHit entry X
  std::vector<double> _adh_entryY;     ///< AuxDetHit entry Y
  std::vector<double> _adh_entryZ;     ///< AuxDetHit entry Z
  std::vector<double> _adh_exitX;      ///< AuxDetHit exit X
  std::vector<double> _adh_exitY;      ///< AuxDetHit exit Y
  std::vector<double> _adh_exitZ;      ///< AuxDetHit exit Z
  ...
  Int_t _isGoodEvent=1; ///< Whether or not the event is good, 1--good events; 0 -- bad events
...
CRTAirTPCAnalysis::CRTAirTPCAnalysis(fhicl::ParameterSet const& p)
  : EDAnalyzer{p}  // ,
  // More initializers here.
{
  ...
  if(!_data_mode){
    ...
    if (_save_mctruth){
      _tree->Branch("nu_e", "std::vector<float>", &_nu_e);
      _tree->Branch("nu_pdg", "std::vector<int>", &_nu_pdg);
      _tree->Branch("nu_ccnc", "std::vector<int>", &_nu_ccnc);
      _tree->Branch("nu_mode", "std::vector<int>", &_nu_mode);
      _tree->Branch("nu_int_type", "std::vector<int>", &_nu_int_type);
      _tree->Branch("nu_vtx_x", "std::vector<float>", &_nu_vtx_x);
      _tree->Branch("nu_vtx_y", "std::vector<float>", &_nu_vtx_y);
      _tree->Branch("nu_vtx_z", "std::vector<float>", &_nu_vtx_z);
      _tree->Branch("nu_px", "std::vector<float>", &_nu_px);
      _tree->Branch("nu_py", "std::vector<float>", &_nu_py);
      _tree->Branch("nu_pz", "std::vector<float>", &_nu_pz);
    ...
    if (_save_mcparticle){
      ...
      _tree->Branch("mcp_t", "std::vector<double>", &_mcp_t);
      ...
      _tree->Branch("mcp_origin", "std::vector<int>", &_mcp_origin);
    }

    _tree->Branch("adh_t", "std::vector<double>", &_adh_t);
    _tree->Branch("adh_t_diff", "std::vector<double>", &_adh_t_diff);
    ...
    _tree->Branch("adh_entryX", "std::vector<double>", &_adh_entryX);
    _tree->Branch("adh_entryY", "std::vector<double>", &_adh_entryY);
    _tree->Branch("adh_entryZ", "std::vector<double>", &_adh_entryZ);
    _tree->Branch("adh_exitX", "std::vector<double>", &_adh_exitX);
    _tree->Branch("adh_exitY", "std::vector<double>", &_adh_exitY);
    _tree->Branch("adh_exitZ", "std::vector<double>", &_adh_exitZ);
  ...
  _tree->Branch("isGoodEvent", &_isGoodEvent, "isGoodEvent/I");
  }
...
void CRTAirTPCAnalysis::analyze(art::Event const& e)
{ 
  ...
  std::vector<art::Ptr<sim::AuxDetHit>> adh_v;   // geant4
  ...
  if(crt_hit_v.size() > 50) _isGoodEvent=0;
  else _isGoodEvent=1;
  ...
  if(!_data_mode){
    ...
    _nu_e.clear(); _nu_pdg.clear(); _nu_ccnc.clear(); _nu_mode.clear(); _nu_int_type.clear();
    _nu_vtx_x.clear(); _nu_vtx_y.clear(); _nu_vtx_z.clear();
    _nu_px.clear(); _nu_py.clear(); _nu_pz.clear();
    ...
    for (size_t imct=0; imct< mct_v.size(); imct++){
      ...
      for (int ipar = 0; ipar < mct->NParticles(); ipar++) {
        ...
        if (_signal_mode){ // dark Neutrino .
          ...
          if (ipar<2){
            ...
            //std::cout<<"Dark neutrino energy: "<<particle.E()<<std::endl;
      ...
      if (mct->Origin() == simb::kBeamNeutrino){
        _nu_e.push_back(mct->GetNeutrino().Nu().E());
        _nu_pdg.push_back(mct->GetNeutrino().Nu().PdgCode());
        _nu_ccnc.push_back(mct->GetNeutrino().CCNC());
        _nu_mode.push_back(mct->GetNeutrino().Mode());
        _nu_int_type.push_back(mct->GetNeutrino().InteractionType());
        _nu_vtx_x.push_back(mct->GetNeutrino().Nu().Vx());
        _nu_vtx_y.push_back(mct->GetNeutrino().Nu().Vy());
        _nu_vtx_z.push_back(mct->GetNeutrino().Nu().Vz());
        _nu_px.push_back(mct->GetNeutrino().Nu().Px());
        _nu_py.push_back(mct->GetNeutrino().Nu().Py());
        _nu_pz.push_back(mct->GetNeutrino().Nu().Pz());

        //std::cout<<"Neutrino info! "<<mct->GetNeutrino().Nu().PdgCode()<<"; neutrino energy: "<<mct->GetNeutrino().Nu().E()<<"; position:("<<mct->GetNeutrino().Nu().Vx()<<", "<<mct->GetNeutrino().Nu().Vy()<<", "<<mct->GetNeutrino().Nu().Vz()<<"); momentum: ("<<mct->GetNeutrino().Nu().Px()<<", "<<mct->GetNeutrino().Nu().Py()<<", "<<mct->GetNeutrino().Nu().Pz()<<"); interaction type: "<<mct->GetNeutrino().InteractionType()<<"; interaction mode: "<<mct->GetNeutrino().Mode()<<std::endl;
      }
    }
    ...
    _adh_t.resize(n_adh); _adh_t_diff.resize(n_adh);
    _adh_deposite_e.resize(n_adh);
    _adh_x.resize(n_adh); _adh_y.resize(n_adh); _adh_z.resize(n_adh);
    _adh_entryX.resize(n_adh); _adh_entryY.resize(n_adh); _adh_entryZ.resize(n_adh);
    _adh_exitX.resize(n_adh);  _adh_exitY.resize(n_adh);  _adh_exitZ.resize(n_adh);
    ...
    for (size_t iadh = 0; iadh < n_adh; iadh++) {
      ...
      _adh_t_diff[iadh] = auxdethit->GetExitT() - auxdethit->GetEntryT();
      ...
      _adh_entryX[iadh]  = auxdethit->GetEntryX(); _adh_entryY[iadh]  = auxdethit->GetEntryY(); _adh_entryZ[iadh]  = auxdethit->GetEntryZ();
      _adh_exitX[iadh]   = auxdethit->GetExitX();  _adh_exitY[iadh]   = auxdethit->GetExitY();  _adh_exitZ[iadh]   = auxdethit->GetExitZ();
      ...
      for (size_t ipar = 0; ipar < mcp_v.size(); ipar++) { // loop through MCParticle to find the pdg number. 
        ...
        if (particle->TrackId() == (int)auxdethit->GetTrackID()){
          ...
          //std::cout<<"ADH! "<<iadh<<", "<<particle->PdgCode()<<", energy: "<<particle->E()<<", ("<<particle->Vx()<<", "<<particle->Vy()<<", "<<particle->Vz()<<"); momentum: ("<<particle->Px()<<", "<<particle->Py()<<", "<<particle->Pz()<<")"<<std::endl;
        }
      }
    ...
    for (size_t iadi = 0; iadi < n_adi; iadi++) {
      ...
      if (_debug) std::cout << "Adding adh with track ID " << auxdetide->trackID << std::endl;
    }
    ...
    if (_save_mcparticle){
      ...
      for (size_t i = 0; i < mcp_v.size(); i++) {
        ...
        _mcp_origin[counter] = FindNeutrinoSources(e, _g4_label, particle->TrackId(),  _signal_mode);
        ...
        if (_debug) {
          std::cout << counter << ", " << i << "MCP " << _mcp_pdg[counter] << ", trackID = " << _mcp_trackid[counter]
                    ...
                    << " --- end point = (" 
                    << _mcp_endx[counter] << ", " << _mcp_endy[counter] << ", " << _mcp_endz[counter] << "); "
                    << "; origin: " << _mcp_origin[counter]
                    << std::endl;
        }
        counter++;
      }
  ...
  for (size_t i = 0; i < n_hits; i++) {
    ...
    if (hit->tagger == "volTaggerNorth_0") {
      _chit_plane[i] = 0; // downstream 
    } else if (hit->tagger == "volTaggerSouth_0"){
      _chit_plane[i] = 1; // upstream
    }else {
      _chit_plane[i] = 2; // any other walls.
    }
...
void CRTAirTPCAnalysis::beginSubRun(art::SubRun const& sr) {
  ...
  if (pot_handle.isValid()) {
    ...
    std::cout<<_sr_run<<", "<<_sr_subrun<<", "<<_sr_pot<< " (" << _sr_spills << " spills), begin time: "<<_sr_begintime<<", end time: "<<_sr_endtime<<std::endl;
  } else {
    _sr_pot = 0.;
    _sr_spills = 0.;
  }
...
void CRTAirTPCAnalysis::respondToOpenInputFile(const art::FileBlock& fb){
    _file_name = fb.fileName();
}

int CRTAirTPCAnalysis::FindNeutrinoSources(art::Event const & e, std::string _geant_producer, int geant_track_id, bool _signal_mode) {
  if (_signal_mode) return 0;
...
DEFINE_ART_MODULE(CRTAirTPCAnalysis)