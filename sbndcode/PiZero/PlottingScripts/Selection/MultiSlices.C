void MultiSlices()
{
  const TString file = "/sbnd/data/users/hlay/ncpizero/production/NCPiZeroA/NCPiZeroA_test_rockbox.root";
              ...
              if(slc_true_event_type->at(j) == 0)
                {
                  std::cout << "Slice with completeness " << slc_comp->at(j)
                            ...
                            << "OpT0Frac: " << (slc_opt0_hypPE->at(j) - slc_opt0_measPE->at(j)) / slc_opt0_measPE->at(j)
                            << std::endl;
                }
            }
