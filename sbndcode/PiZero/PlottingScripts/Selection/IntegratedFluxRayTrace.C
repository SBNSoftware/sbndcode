...
void IntegratedFluxRayTrace(const TString productionVersion)
{
  TString saveDir = baseSaveDir + "/" + productionVersion + "/integrated_flux_ray_trace";
  ...
  for(auto&& [ weight_i, name ] : enumerate(weight_names))
    {
      nus->SetBranchStatus(Form("evtwgt_flux_weight_%s", name.c_str()), 1);
      nus->SetBranchAddress(Form("evtwgt_flux_weight_%s", name.c_str()), &weights[weight_i]);
    }

  nus->SetBranchStatus("evtwgt_flux_oneweight", 1);
  ...
  for(int i = 0; i < N; ++i)
    {
      ...
      if(ray_fv_length > 0)
        {
          const double w = ray_fv_length / 440;

          hNuEnergyNominal->Fill(nu_e, w);
          nominalCount += w;
