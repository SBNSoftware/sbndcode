...
namespace opdet {
    ...
    , fEngine(fParams.engine)
  {
    ...
    xvec=AssignVector(fParams.TPBFile);
    timeTPB = new TH1D("Time TPB", "", xvec.size(), 0.0, 1.0*xvec.size());//histogram that stores the emission time of photons converted by TPB
  ...
  std::vector<double> DigiPMTSBNDAlg::AssignVector(std::string filename){
    ...
    mf::LogInfo("DigiArapucaSBNDAlg")<<"DigiArapuca opening file " << fname;
   ...
   else throw cet::exception("DigiPMTSBNDAlg") << "No File: Unable to open file " << fname << "\n";
  ...
  (Config const& config)
  {
    ...
    fBaseConfig.TPBFile                  = config.tpbFile();
    fBaseConfig.TestBenchPEFile          = config.testbenchpeFile();
  }
