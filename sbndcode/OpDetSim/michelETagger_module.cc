...
void michelETagger::ConvolveWithAnyKernel(std::vector<double> &Waveform, std::vector<double> &Kernel, std::vector<double> &Out) {
    ...
    {
        double PointSum = 0;
        for (int Index : std::vector<int>(X_indices.begin(), X_indices.end() - KernelSize/2 + i) ) //end is samples +1 so the -1 shows up
        {
            PointSum += Waveform[i - Index] * Kernel[KernelSize/2 + Index];
        }
        Out[i] = PointSum;
...
bool michelETagger::CheckForMichelCoincidence(double WaveformTimestamp, int MuonSample, art::Handle< std::vector<sbnd::crt::CRTCluster>> &crtClusterHandle)
{
  ...
  for(int ClustID; ClustID<int(crtClusterHandle->size()); ClustID++)
  {
    ...
    if(TDiff>0 && TDiff<=fCoincidentWindow) 
    {
      std::cout << " coinci found " <<  TDiff << std::endl;
      CoincidentCRT=true;
    }
