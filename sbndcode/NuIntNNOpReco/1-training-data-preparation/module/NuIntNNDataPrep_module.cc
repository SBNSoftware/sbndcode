...
// ── Temporal preprocessing constants (match preprocess_temporal.py) ──────────
namespace {
  constexpr float kPadValue      = -1000.0f;
  constexpr float kCLightCmNs    = 29.9792458f;          // cm/ns vacuum
  constexpr float kGeomXMax      = 213.75f;              // cm  PMT x range
  constexpr float kGeomYMax      = 175.00f;              // cm  PMT y range
  constexpr float kGeomZMin      =  16.05f;              // cm  PMT z range
  constexpr float kGeomZMax      = 484.95f;              // cm
  constexpr int   kNFeatures     = 7;
}
...
namespace opdet {
  ...
  struct ProcessedData_DP {
    std::vector<float>              flash_time_sel;
    ...
    std::vector<float>              flash_time_final;
...
namespace opdet { class NuIntNNDataPrep; }

class opdet::NuIntNNDataPrep : public art::EDAnalyzer {
public:
  explicit NuIntNNDataPrep(fhicl::ParameterSet const& p);

  NuIntNNDataPrep(NuIntNNDataPrep const&) = delete;
  NuIntNNDataPrep(NuIntNNDataPrep&&) = delete;
  NuIntNNDataPrep& operator=(NuIntNNDataPrep const&) = delete;
  NuIntNNDataPrep& operator=(NuIntNNDataPrep&&) = delete;
...
private:
  ...
  std::vector<double>      fFlashTimeWindow;  // µs: [min, max] for OpFlash AbsTime selection
  ...
  int                      fTemporalMaxOphits;       // ophits per flash kept in sequence
  int                      fTemporalNumFlashes;      // flashes concatenated per event
  bool                     fTemporalUseEnergyVertex; // false=MC vertex, true=dEprom centroid
  ...
  // ─── PMT position LUT (indexed by channel id) ────────────────────────────
  std::vector<float> fLutPmtX, fLutPmtY, fLutPmtZ;

  // ─── TTree and its branch variables ─────────────────────────────────────
  // Four TTrees in separate subdirectories for selective loading in Python:
  ...
  // All trees share run/subrun/event for joining.
  TTree* fLabelsTree;
  TTree* fTemporalTree;
  TTree* fOphitsTree;
  TTree* fImagesTree;
  ...
  // Per-flash data (final selection)
  std::vector<float>              fFlashTime;
  ...
  // Temporal sequence: flat (seq_len * 7) features + mask
  std::vector<float>   fTemporalFeatures; // seq_len * kNFeatures, row-major
  std::vector<int>     fTemporalMask;     // seq_len: 1=real ophit, 0=padding
  int                  fTemporalSeqLen;   // = fTemporalNumFlashes * fTemporalMaxOphits
};
...
// Constructor
// ============================================================================
opdet::NuIntNNDataPrep::NuIntNNDataPrep(fhicl::ParameterSet const& p)
  : EDAnalyzer{p},
    ...
    fFlashTimeWindow     ( p.get<std::vector<double>>("FlashTimeWindow", {0.15, 1.75}) ),
...
// ============================================================================
void opdet::NuIntNNDataPrep::beginJob()
{
  art::ServiceHandle<art::TFileService> tfs;
  fTemporalSeqLen = fTemporalNumFlashes * fTemporalMaxOphits;

  // ─── labels/tree: event ID + MC truth + training labels ──────────────────
  art::TFileDirectory dirLabels = tfs->mkdir("labels");
  fLabelsTree = dirLabels.make<TTree>("tree", "Event ID, MC truth and training labels");
  fLabelsTree->Branch("run",            &fRun,           "run/I");
  fLabelsTree->Branch("subrun",         &fSubrun,        "subrun/I");
  fLabelsTree->Branch("event",          &fEvent,         "event/I");
  fLabelsTree->Branch("passed_filters", &fPassedFilters, "passed_filters/O");
  fLabelsTree->Branch("selected_tpc",   &fSelectedTPC,   "selected_tpc/I");
  fLabelsTree->Branch("nuv_x",   &fNuvX);
  fLabelsTree->Branch("nuv_y",   &fNuvY);
  fLabelsTree->Branch("nuv_z",   &fNuvZ);
  fLabelsTree->Branch("nuv_t",   &fNuvT);
  fLabelsTree->Branch("nuv_e",   &fNuvE);
  fLabelsTree->Branch("nuv_pdg", &fNuvPDG);
  fLabelsTree->Branch("dEprom_x",   &fDEpromX);
  fLabelsTree->Branch("dEprom_y",   &fDEpromY);
  fLabelsTree->Branch("dEprom_z",   &fDEpromZ);
  fLabelsTree->Branch("dEtpc",      &fDEtpc);
  fLabelsTree->Branch("dEdir_x",    &fDEdirX);
  fLabelsTree->Branch("dEdir_y",    &fDEdirY);
  fLabelsTree->Branch("dEdir_z",    &fDEdirZ);
  fLabelsTree->Branch("dEspread_x", &fDEspreadX);
  fLabelsTree->Branch("dEspread_y", &fDEspreadY);
  fLabelsTree->Branch("dEspread_z", &fDEspreadZ);
  // PCA eigenvalues (λ1≥λ2≥λ3, cm², energy-weighted covariance; λ1 eigenvector = dEdir_x/y/z)
  fLabelsTree->Branch("dEpca_lam1", &fDEpcaLam1);
  fLabelsTree->Branch("dEpca_lam2", &fDEpcaLam2);
  fLabelsTree->Branch("dEpca_lam3", &fDEpcaLam3);
  // Secondary (v2) and tertiary (v3) PCA eigenvectors
  fLabelsTree->Branch("dEpca_v2x",  &fDEpcaV2x);
  fLabelsTree->Branch("dEpca_v2y",  &fDEpcaV2y);
  fLabelsTree->Branch("dEpca_v2z",  &fDEpcaV2z);
  fLabelsTree->Branch("dEpca_v3x",  &fDEpcaV3x);
  fLabelsTree->Branch("dEpca_v3y",  &fDEpcaV3y);
  fLabelsTree->Branch("dEpca_v3z",  &fDEpcaV3z);

  // ─── temporal/tree: temporal sequence features ───────────────────────────
  art::TFileDirectory dirTemporal = tfs->mkdir("temporal");
  fTemporalTree = dirTemporal.make<TTree>("tree", "Temporal sequence features");
  fTemporalTree->Branch("run",              &fRun,             "run/I");
  fTemporalTree->Branch("subrun",           &fSubrun,          "subrun/I");
  fTemporalTree->Branch("event",            &fEvent,           "event/I");
  fTemporalTree->Branch("temporal_features", &fTemporalFeatures);
  fTemporalTree->Branch("temporal_mask",     &fTemporalMask);
  fTemporalTree->Branch("temporal_seq_len",  &fTemporalSeqLen, "temporal_seq_len/I");

  // ─── ophits/tree: per-flash optical hit data ─────────────────────────────
  art::TFileDirectory dirOphits = tfs->mkdir("ophits");
  fOphitsTree = dirOphits.make<TTree>("tree", "Per-flash optical hit data");
  fOphitsTree->Branch("run",    &fRun,    "run/I");
  fOphitsTree->Branch("subrun", &fSubrun, "subrun/I");
  fOphitsTree->Branch("event",  &fEvent,  "event/I");
  fOphitsTree->Branch("flash_time",       &fFlashTime);
  fOphitsTree->Branch("flash_ophit_pe",   &fFlashOphitPE);
  fOphitsTree->Branch("flash_ophit_ch",   &fFlashOphitCh);
  fOphitsTree->Branch("flash_ophit_time", &fFlashOphitTime);

  // ─── images/tree: 2D images + pe_per_channel ─────────────────────────────
  art::TFileDirectory dirImages = tfs->mkdir("images");
  fImagesTree = dirImages.make<TTree>("tree", "2D PMT images and PE per channel");
  fImagesTree->Branch("run",    &fRun,    "run/I");
  fImagesTree->Branch("subrun", &fSubrun, "subrun/I");
  fImagesTree->Branch("event",  &fEvent,  "event/I");
  fImagesTree->Branch("pe_per_channel",  &fPePerChannel);
  fImagesTree->Branch("image_uncoated",  &fImageUncoated);
  fImagesTree->Branch("image_coated",    &fImageCoated);
  fImagesTree->Branch("image_ny",        &fImageNy,       "image_ny/I");
  fImagesTree->Branch("image_nz",        &fImageNz,       "image_nz/I");
  fImagesTree->Branch("max_pe_uncoated", &fMaxPeUncoated, "max_pe_uncoated/F");
  fImagesTree->Branch("max_pe_coated",   &fMaxPeCoated,   "max_pe_coated/F");

  if(fVerbosity > 0)
    std::cout << "[DataPrep] TTrees initialized in 4 subdirs (labels/temporal/ophits/images)"
              << "  seq_len=" << fTemporalSeqLen << " (" << fTemporalNumFlashes
              << " flashes × " << fTemporalMaxOphits << " ophits)" << std::endl;

  // ─── PDSMapTree: one entry per optical channel (geometry + type) ──────────
  fPDSMapTree = tfs->make<TTree>("PDSMapTree", "PDS channel geometry map");
  int   pdsID, pdsType;
  float pdsX, pdsY, pdsZ;
  fPDSMapTree->Branch("OpDetID",   &pdsID,   "OpDetID/I");
  fPDSMapTree->Branch("OpDetType", &pdsType, "OpDetType/I");
  fPDSMapTree->Branch("OpDetX",    &pdsX,    "OpDetX/F");
  fPDSMapTree->Branch("OpDetY",    &pdsY,    "OpDetY/F");
  fPDSMapTree->Branch("OpDetZ",    &pdsZ,    "OpDetZ/F");

  art::ServiceHandle<geo::Geometry const> geo;
  for(unsigned int ch = 0; ch < geo->NOpDets(); ++ch) {
    pdsID   = static_cast<int>(ch);
    pdsType = fChannelDict.count(ch) ? fChannelDict.at(ch) : -1;
    auto center = geo->OpDetGeoFromOpDet(ch).GetCenter();
    pdsX = static_cast<float>(center.X());
    pdsY = static_cast<float>(center.Y());
    pdsZ = static_cast<float>(center.Z());
    fPDSMapTree->Fill();
  }
  if(fVerbosity > 0)
    std::cout << "[DataPrep] PDSMapTree filled: " << geo->NOpDets() << " channels" << std::endl;

  // ─── Build PMT position LUT for temporal per-channel ToF correction ───────
  fLutPmtX.assign(geo->NOpDets(), 0.0f);
  fLutPmtY.assign(geo->NOpDets(), 0.0f);
  fLutPmtZ.assign(geo->NOpDets(), 0.0f);
  for(unsigned int ch = 0; ch < geo->NOpDets(); ++ch) {
    auto center  = geo->OpDetGeoFromOpDet(ch).GetCenter();
    fLutPmtX[ch] = static_cast<float>(center.X());
    fLutPmtY[ch] = static_cast<float>(center.Y());
    fLutPmtZ[ch] = static_cast<float>(center.Z());
  }
}
...
// ============================================================================
void opdet::NuIntNNDataPrep::analyze(art::Event const& e)
{
  ...
  // ─── OpFlashes (beam window from FCL: FlashTimeWindow [µs]) ─────────────
  ...
  for(size_t s = 0; s < fOpFlashesModuleLabel.size(); ++s) {
    ...
    for(unsigned int i = 0; i < opflashListHandle->size(); ++i) {
      ...
      if(flash_time_us < fFlashTimeWindow[0] || flash_time_us > fFlashTimeWindow[1]) continue;
      ...
      fOpticalData.flash_time.push_back(static_cast<float>(flash_time_us));
...
// ============================================================================
void opdet::NuIntNNDataPrep::CreateRawPEImages()
{
...
// BuildTemporalSequence
//
// Mirrors preprocess_temporal.py steps 3-6 (ToF variant: t_ophit − nuvZ/c):
//   1. Collect ophits from selected flashes, sort by time (ascending)
//   2. Pad to fixed seq_len = NumFlashes × MaxOphits
//   3. Correct ophit times: subtract nuvZ/c (neutrino ToF inside detector)
//   4. Build 7-feature tensor per ophit slot (kNFeatures=7):
//        [t_us, PE_raw, det_type, x_norm, y_norm, z_norm, delta_t_us]
//        (PE saved raw; logPE normalization applied in Python using training-set pe_max)
//   5. Fill fTemporalFeatures (flat seq_len*7) and fTemporalMask (seq_len)
// ============================================================================
void opdet::NuIntNNDataPrep::BuildTemporalSequence()
{
  const int seqLen = fTemporalSeqLen;

  // Pre-size outputs (padding values)
  fTemporalFeatures.assign(seqLen * kNFeatures, kPadValue);
  fTemporalMask.assign(seqLen, 0);

  // ── Vertex Z for ToF correction (nuvZ/c) ─────────────────────────────────
  // TemporalUseEnergyVertex=false (default): MC true neutrino vertex in AV
  // TemporalUseEnergyVertex=true           : dEprom energy-weighted centroid
  float vtxX = 0.0f, vtxY = 0.0f, vtxZ = 0.0f;
  (void)vtxX; (void)vtxY;  // only vtxZ used for ToF correction
  bool  foundVtx = false;

  if(fTemporalUseEnergyVertex) {
    // dEprom_x/y/z_final: selected TPC energy-weighted centroid (first entry)
    if(!fProcessedData.dEpromx_final.empty() &&
       fProcessedData.dEpromx_final[0] != fDefaultSimIDE) {
      vtxX     = static_cast<float>(fProcessedData.dEpromx_final[0]);
      vtxY     = static_cast<float>(fProcessedData.dEpromy_final[0]);
      vtxZ     = static_cast<float>(fProcessedData.dEpromz_final[0]);
      foundVtx = true;
    }
  } else {
    // MC truth vertex: find the neutrino in the active volume
    for(size_t i = 0; i < fMCData.nuvX.size(); ++i) {
      bool inAV = (fMCData.nuvX[i] >= fActiveVolumeX[0] && fMCData.nuvX[i] <= fActiveVolumeX[1]) &&
                  (fMCData.nuvY[i] >= fActiveVolumeY[0] && fMCData.nuvY[i] <= fActiveVolumeY[1]) &&
                  (fMCData.nuvZ[i] >= fActiveVolumeZ[0] && fMCData.nuvZ[i] <= fActiveVolumeZ[1]);
      if(inAV) {
        vtxX     = static_cast<float>(fMCData.nuvX[i]);
        vtxY     = static_cast<float>(fMCData.nuvY[i]);
        vtxZ     = static_cast<float>(fMCData.nuvZ[i]);
        foundVtx = true;
        break;
      }
    }
  }
  if(!foundVtx) return;  // should not happen after filters

  const float tofInsideUs = vtxZ / kCLightCmNs / 1000.0f;  // nuvZ/c in μs

  // ── Collect and sort ophits from selected flashes ─────────────────────────
  // flash_ophit_pe/ch/time_final: vector<vector<>> of selected TPC flashes
  struct OphitEntry { float t_us; float pe; int ch; };
  const int maxNch = static_cast<int>(fLutPmtX.size());

  int slotFilled = 0;

  const int nFlashes = static_cast<int>(fProcessedData.flash_ophit_time_final.size());
  for(int fl = 0; fl < std::min(nFlashes, fTemporalNumFlashes); ++fl) {
    const auto& times = fProcessedData.flash_ophit_time_final[fl];
    const auto& pes   = fProcessedData.flash_ophit_pe_final[fl];
    const auto& chs   = fProcessedData.flash_ophit_ch_final[fl];

    // Build sorted index array for this flash, removing unphysical ophits
    // (t_us < 0 after ToF correction means the hit predates the neutrino — noise/cosmic)
    int nOph = static_cast<int>(times.size());
    std::vector<int> idx(nOph);
    std::iota(idx.begin(), idx.end(), 0);
    std::sort(idx.begin(), idx.end(), [&](int a, int b){ return times[a] < times[b]; });
    idx.erase(std::remove_if(idx.begin(), idx.end(),
        [&](int i){ return (times[i] - tofInsideUs) < 0.0f; }), idx.end());

    int slotBase = fl * fTemporalMaxOphits;
    int taken    = std::min(static_cast<int>(idx.size()), fTemporalMaxOphits);

    for(int k = 0; k < taken; ++k) {
      int   i     = idx[k];
      int   ch    = chs[i];
      float t_us  = times[i] - tofInsideUs;  // ToF-corrected, guaranteed >= 0
      float pe    = pes[i];
      int   slot  = slotBase + k;

      // ── Step 4: build features ───────────────────────────────────────────
      // PE saved raw (un-normalized); logPE normalization done in Python
      // using pe_max computed from training set (consistent with image handling)
      float detType = (ch >= 0 && ch < maxNch && fChannelDict.count(ch))
                      ? static_cast<float>(fChannelDict.at(ch)) : kPadValue;
      float xNorm = (ch >= 0 && ch < maxNch)
                    ? fLutPmtX[ch] / kGeomXMax : kPadValue;
      float yNorm = (ch >= 0 && ch < maxNch)
                    ? fLutPmtY[ch] / kGeomYMax : kPadValue;
      float zNorm = (ch >= 0 && ch < maxNch)
                    ? (fLutPmtZ[ch] - kGeomZMin) / (kGeomZMax - kGeomZMin) : kPadValue;

      int base = slot * kNFeatures;
      fTemporalFeatures[base + 0] = t_us;
      fTemporalFeatures[base + 1] = pe;  // raw PE, normalize in Python
      fTemporalFeatures[base + 2] = detType;
      fTemporalFeatures[base + 3] = xNorm;
      fTemporalFeatures[base + 4] = yNorm;
      fTemporalFeatures[base + 5] = zNorm;
      // feature 6 (delta_t) filled in second pass below
      fTemporalMask[slot] = 1;
      ++slotFilled;
    }
  }

  // ── Step 5 cont: feature 6 = delta_t = t_us - t_first ────────────────────
  // Find first real ophit time across all slots
  float tFirst = std::numeric_limits<float>::max();
  for(int s = 0; s < seqLen; ++s) {
    if(fTemporalMask[s]) {
      float t = fTemporalFeatures[s * kNFeatures + 0];
      if(t < tFirst) tFirst = t;
    }
  }
  if(tFirst == std::numeric_limits<float>::max()) tFirst = 0.0f;

  for(int s = 0; s < seqLen; ++s) {
    if(fTemporalMask[s])
      fTemporalFeatures[s * kNFeatures + 6] = fTemporalFeatures[s * kNFeatures + 0] - tFirst;
    // padding slots keep kPadValue (already set)
  }

  if(fVerbosity > 1)
    std::cout << "[DataPrep] TemporalSeq: " << slotFilled << "/" << seqLen
              << " ophits filled  tFirst=" << tFirst << " μs" << std::endl;
}
...
// FillTrainingTree
// ============================================================================
void opdet::NuIntNNDataPrep::FillTrainingTree(bool passedFilters)
{
  ...
  // PE flat per channel + flash ophit data + images + temporal: only meaningful if passedFilters
  ...
  fImageCoated.clear();
  if(!passedFilters) {
    fTemporalFeatures.assign(fTemporalSeqLen * kNFeatures, kPadValue);
    fTemporalMask.assign(fTemporalSeqLen, 0);
  }
  ...
  if(passedFilters) {
    ...
    // Per-flash selected data
    fFlashTime      = fProcessedData.flash_time_final;
    ...
    // Temporal features: already filled by BuildTemporalSequence() (called before FillTrainingTree)
    // fTemporalFeatures and fTemporalMask are set in-place there; nothing to copy here
  }
...
// ============================================================================
void opdet::NuIntNNDataPrep::FillMCTruth(art::Event const& e)
{
...
// ============================================================================
void opdet::NuIntNNDataPrep::FillAverageDepositedEnergyVariables(
  std::vector<std::vector<double>> fenergydep,
  std::vector<std::vector<double>> fenergydepX,
  std::vector<std::vector<double>> fenergydepY,
  std::vector<std::vector<double>> fenergydepZ,
...
// ============================================================================
void opdet::NuIntNNDataPrep::InitializeChannelDict()
{
...
// ============================================================================
void opdet::NuIntNNDataPrep::ClassifyChannels()
{
...
// ============================================================================
int opdet::NuIntNNDataPrep::CategorizeFirstChannel(const std::vector<int>& channels)
{
...
// ============================================================================
void opdet::NuIntNNDataPrep::LoadPMTMaps()
{
  auto searchAndOpen = [&](const std::string& basePath, const std::string& label) -> std::string {
    ...
    if(!test.is_open()) {
      ...
      // Search FW_SEARCH_PATH (set by UPS for scripts/ directory in tarball and local products)
    std::string fw_found = "";
    try {
      cet::search_path sp("FW_SEARCH_PATH");
      sp.find_file("PosRecoCVN/pmt_maps/" + filename, fw_found);
    } catch(...) {}
...
// ============================================================================
std::vector<std::vector<float>> opdet::NuIntNNDataPrep::SelectNonEmptyHalf(
    const std::vector<std::vector<float>>& left_half,
    const std::vector<std::vector<float>>& right_half,
...
// ============================================================================
void opdet::NuIntNNDataPrep::ApplyFlashSelection()
{
  fProcessedData.flash_time_sel.clear();
  ...
  for(size_t i = 0; i < fOpticalData.flash_time.size(); ++i)
    if(i < selected_mask.size() && selected_mask[i])
      fProcessedData.flash_time_sel.push_back(fOpticalData.flash_time[i]);
...
// ============================================================================
void opdet::NuIntNNDataPrep::ApplyFinalEnergyFilter()
{
  fProcessedData.flash_time_final.clear();
  ...
  if(passFilter) {
    fProcessedData.flash_time_final       = fProcessedData.flash_time_sel;
...
// ============================================================================
void opdet::NuIntNNDataPrep::CreatePEMatrix()
{
...
// ClearEventData
// ============================================================================
void opdet::NuIntNNDataPrep::ClearEventData()
{
  fEventInfo.clear();
  fMCData.clear();
  fOpticalData.clear();
...
// ============================================================================
template<typename T>
std::vector<std::vector<T>> opdet::NuIntNNDataPrep::FilterByMask(
    const std::vector<std::vector<T>>& array,
    const std::vector<std::vector<bool>>& mask)
{
...
// LogTiming
// ============================================================================
void opdet::NuIntNNDataPrep::LogTiming(
    const std::string& op,
    const std::chrono::time_point<std::chrono::high_resolution_clock>& t0)
{
  if(fVerbosity > 0) {
...
DEFINE_ART_MODULE(opdet::NuIntNNDataPrep)
