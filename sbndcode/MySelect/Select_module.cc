...
private:
  ...
  double		pfpT0TriggerBits;
  double		pfpT0TriggerType;
  double		pfpT0ID;
  double 		pfpCRTHitPESHit;
  double   		pfpCRTHitT0; 
  ...
  std::vector< art::Ptr<recob::PFParticle> > Get_CRT(
	const std::vector< art::Ptr<recob::PFParticle> > &pfps, 
	const art::FindManyP<recob::Track> &trackAssoc, 
	const art::FindManyP<anab::T0> &t0Assoc) const;
...
//  std::cout << "Efield: " << detectorData.Efield(0) << std::endl;  
  ...
  if(e.getByLabel(fT0Label, T0Handle)){
    art::fill_ptr_vector(T0List, T0Handle);
  }
  ...
  art::FindManyP<sbn::crt::CRTHit> crthitAssoc (T0List, e, fCRTHitLabel);
...
//      pfpT0TriggerBits = t0->TriggerBits();
//      pfpT0TriggerType = t0->TriggerType();
//      pfpT0ID = t0->ID();
//
//
//    } 
    ...
    for(const art::Ptr<recob::Track> &trk: pfpTrack){
      ...
      if(trkT0.empty()) continue;

      for(const art::Ptr<anab::T0> &t0: trkT0){
        ...
        std::vector< art::Ptr<sbn::crt::CRTHit> > t0CRT = crthitAssoc.at(t0.key());
        ...
        for(const art::Ptr<sbn::crt::CRTHit> &crthit: t0CRT){
          ...
          pfpCRTHitT0 = ((double) crthit->ts1_ns) / 1000000; //unit: ns -> Convert ns to ms: /10^6 
          pfpCRTHitPlane = crthit->plane;     
...
void sbnd::Select::beginJob()
{
  ...
  pfpTree->Branch("T0CRTPlane", &pfpT0TriggerBits, "T0CRTPlane/D");
  pfpTree->Branch("T0CRTTrackID", &pfpT0TriggerType, "T0CRTTrackID/D");
  pfpTree->Branch("T0CRTExtrapolationLength", &pfpT0ID, "T0CRTExtrapolationLength/D");
  pfpTree->Branch("CRTHitPESHit", &pfpCRTHitPESHit, "CRTHitPESHit/D");
...
//-------------------------------------------------------------------
// Get pfparticle that crt tracks  

std::vector< art::Ptr<recob::PFParticle> > sbnd::Select::Get_CRT(const std::vector< art::Ptr<recob::PFParticle> > &pfps, const art::FindManyP<recob::Track> &trackAssoc, const art::FindManyP<anab::T0> &t0Assoc) const
{
  std::vector< art::Ptr<recob::PFParticle> > crosser;
  crosser.clear();
  ...
  for(const art::Ptr<recob::PFParticle> &pfp: pfps){
    ...
    for(const art::Ptr<recob::Track> &trk: pfpTrack){
      ...
      std::vector< art::Ptr<anab::T0> > trkT0 = t0Assoc.at(trk.key()); //check if T0 reco 

      if(trkT0.empty()) continue; //check if T0 is reco 
...
void sbnd::Select::clearPfpTree()
{
  ...
  pfpT0TriggerBits = 99999;
  pfpT0TriggerType = 99999;
  pfpT0ID = 99999;
  pfpCRTHitPESHit = 99999;
  pfpCRTHitT0 = 99999; 
