...
class ACProducer: public art::EDProducer {
public:
    // The destructor generated by the compiler is fine for classes
    // without bare pointers or other resource use.

    // Plugins should not be copied or assigned.
   explicit ACProducer(fhicl::ParameterSet const & p);
   ACProducer(ACProducer const &) = delete;
   ACProducer(ACProducer &&) = delete;
   ACProducer & operator = (ACProducer const &) = delete;
   ACProducer & operator = (ACProducer &&) = delete;
...
private:
   ...
   // Performs Hough Transform for collection planes (also stores collection plane hits)
...
ACProducer::ACProducer(fhicl::ParameterSet const & p)
   : EDProducer(p)
{
...
// Constructor
void ACProducer::reconfigure(fhicl::ParameterSet const & p)
{  
...
void ACProducer::produce(art::Event & evt)
{
   std::unique_ptr<vector<sbn::crt::CRTTrack> > ac_tracks(new vector<sbn::crt::CRTTrack>);
   ...
   if (ac_in_tpc0 == true || ac_in_tpc1 == true){
      ...
      if (ac_in_tpc0){
         //std::cout << "AC muon found in event " << event << ", in TPC 0" <<std::endl;
         ...
         if (lines_00.empty() == false){
            ...
            if (lines_01.empty()==false){
               ...
               std::cout << "tpc0: induction lines not found for AC muon" << std::endl;
            }
         }
      }
      if (ac_in_tpc1){
         //std::cout << "AC muon found in event " << event << ", in TPC 1" <<std::endl;
         ...
         if (lines_10.empty() == false){
            ...
            if (lines_11.empty() == false){
               ...
               std::cout << "tpc1: induction lines not found for AC muon" << std::endl;
            }
         }
      }
   ...
   evt.put(std::move(ac_tracks));
} // ACProducer::produce()

float ACProducer::Distance(int x1, int y1, int x2, int y2){
   return sqrt(pow(x2 - x1, 2) + pow(y2 - y1, 2) * 1.0);
}

void ACProducer::Hough_col(vector<vector<int>> coords, int threshold, int max_gap, int range, float min_length, int muon_length, int nentry, 
                       vector<vector<int>>& lines, vector<vector<int>>& colwire, vector<vector<int>>& colpeakT){
...
void ACProducer::Hough_ind(vector<vector<int>> coords, int threshold, int max_gap, int range, float min_length, int muon_length, int nentry, vector<vector<int>>& lines){
   //set global variables 
   TRandom3 rndgen;
   const int h = 3500; const int w = 2000; //range of hit_wire
   constexpr int accu_h = h + w + 1 ; const int accu_w = 180; 
...
void ACProducer::ResetCollectionHitVectors() {
...
void ACProducer::ResetInductionHitVectors(){
   hit_00.clear();
   hit_01.clear();
   hit_10.clear();
   hit_11.clear();
...
void ACProducer::ResetACVariables(){
   ac_t0.clear();
   ac_endpoints.clear();
...
// void ACProducer::FindCollectionPlaneHits(vector<vector<int>> colwire, vector<vector<int>> colpeakT){
...
void ACProducer::Findt0(vector<vector<int>> lines,
                        vector<double>& ac_t0){
...
void ACProducer::FindEndpoints( vector<vector<int>> lines_col, vector<vector<int>> lines_ind, 
...
void ACProducer::FindTrajectories(vector<vector<geo::Point_t>> ac_endpoints, vector<vector<double>>& ac_trajectories){
