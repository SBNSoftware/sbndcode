...
public:
   ...
   explicit testProducer(fhicl::ParameterSet const & p);
   testProducer(testProducer const &) = delete;
   testProducer(testProducer &&) = delete;
   testProducer & operator = (testProducer const &) = delete;
   testProducer & operator = (testProducer &&) = delete;
...
testProducer::testProducer(fhicl::ParameterSet const & p)
   : EDProducer(p)
{
...
// Constructor
void testProducer::reconfigure(fhicl::ParameterSet const & p)
{  
...
void testProducer::produce(art::Event & evt)
{
...
float testProducer::Distance(int x1, int y1, int x2, int y2){
   return sqrt(pow(x2 - x1, 2) + pow(y2 - y1, 2) * 1.0);
}

void testProducer::Hough_col(vector<vector<int>> coords, int threshold, int max_gap, int range, float min_length, int muon_length, int nentry, 
                       vector<vector<int>>& lines, vector<vector<int>>& colwire, vector<vector<int>>& colpeakT){
...
void testProducer::Hough_ind(vector<vector<int>> coords, int threshold, int max_gap, int range, float min_length, int muon_length, int nentry, vector<vector<int>>& lines){
   //set global variables 
   TRandom3 rndgen;
   const int h = 3500; const int w = 2000; //range of hit_wire
   constexpr int accu_h = h + w + 1 ; const int accu_w = 180; 
...
void testProducer::ResetCollectionHitVectors() {
...
void testProducer::ResetInductionHitVectors(){
   hit_00.clear();
   hit_01.clear();
   hit_10.clear();
   hit_11.clear();
...
void testProducer::ResetACVariables(){
   ac_t0.clear();
   ac_endpoints.clear();
...
// void testProducer::FindCollectionPlaneHits(vector<vector<int>> colwire, vector<vector<int>> colpeakT){
...
void testProducer::Findt0(vector<vector<int>> lines,
                        vector<double>& ac_t0){
...
void testProducer::FindEndpoints( vector<vector<int>> lines_col, vector<vector<int>> lines_ind, 
...
void testProducer::FindTrajectories(vector<vector<geo::Point_t>> ac_endpoints, vector<vector<double>>& ac_trajectories){
...
DEFINE_ART_MODULE(testProducer)