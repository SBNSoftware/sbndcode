...
class nuclearFragments::OldFilter : public art::EDFilter {
public:
  explicit OldFilter(fhicl::ParameterSet const& p);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  OldFilter(OldFilter const&) = delete;
  OldFilter(OldFilter&&) = delete;
  OldFilter& operator=(OldFilter const&) = delete;
  OldFilter& operator=(OldFilter&&) = delete;
...
nuclearFragments::OldFilter::OldFilter(fhicl::ParameterSet const& p)
  : EDFilter{p},
...
bool nuclearFragments::OldFilter::filter(art::Event& e)
{ 
  // Set the event ID
  fEventID = e.id().event();
  ResetVariables();

  std::cout << "\n \n Event : " << fEventID << std::endl;
  std::cout << fCountRecoDeuteronWithDesiredP << std::endl;
  std::cout << fCountNonRecoDeuteronWithDesiredP << std::endl;
  ...
  for(const art::Ptr<simb::MCTruth> &truNu : truNuVector){
    ...
    for(const art::Ptr<simb::MCParticle> &particle : particles){
      ...
      std::cout << checkIfReconstructed << std::endl;
...
void nuclearFragments::OldFilter::ResetVariables()
{
...
DEFINE_ART_MODULE(nuclearFragments::OldFilter)