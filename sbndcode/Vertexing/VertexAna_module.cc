...
class VertexAna : public art::EDAnalyzer {
public:
...
private:
  ...
  std::string fNuGenModuleLabel, fCosmicGenModuleLabel, fLArGeantModuleLabel,
    ...
    fSpacePointModuleLabel, // fParticleIDModuleLabel,
    fSliceModuleLabel, fSimChannelModuleLabel;
  ...
  //    tr_chi2Proton, tr_chi2Pion, tr_chi2Muon,
    tr_meanScatter, tr_stdDevScatter;
...
VertexAna::VertexAna(fhicl::ParameterSet const& p)
  : EDAnalyzer{p},
...
void VertexAna::SimulationProcessor(art::Event const &e)
{
  eRunID = e.run(); 
  eSubRunID = e.subRun();
  eEventID = e.event();
  ...
  for(int nu_i = 0; nu_i < nNeutrinos; ++nu_i){
    ...
    if(truthNeutrino->Origin() == 1) {
      ...
      xCorrection.push_back( propD.ConvertTicksToX(clockData.TPCG4Time2Tick(neutrinoParticle.T())- clockData.Time2Tick(clockData.BeamGateTime()),0,0,0) - propD.ConvertTicksToX(0,0,0,0) );
...
void VertexAna::MCParticleProcessor(art::Event const &e, 
                                    art::Ptr<simb::MCParticle> const &particle, 
                                    bool const &isCosmic) 
{
...
void VertexAna::ReconstructionProcessor(art::Event const &e)
{
  ...
  for(unsigned int slice_i = 0; slice_i < eHandleSlices->size(); ++slice_i){
    ...
    if(nuPrimary->PdgCode() != 12 && nuPrimary->PdgCode() != 14)
      continue;

    std::vector<art::Ptr<recob::Vertex> > pfpVertices = pfpVertexAssoc.at(nuPrimary.key());
    if(pfpVertices.size() != 1) {std::cout << "Multiple vertices!!!" << std::endl; continue;}
...
void VertexAna::TrackProcessor(art::Event const &e,
                               art::Ptr<recob::PFParticle> const &pfp,
                               art::Ptr<recob::Track> const &track)
{
  bool isPrimary = false;
...
void VertexAna::ShowerProcessor(art::Event const &e,
                                art::Ptr<recob::PFParticle> const &pfp,
                                art::Ptr<recob::Shower> const &shower)
{
  bool isPrimary = false;
...
void VertexAna::HitProcessor(art::Event const &e, int const &nuID)
{
...
void VertexAna::SliceHits(art::Event const &e, std::vector<art::Ptr<recob::Hit> > const &sliceHits)
{
...
void VertexAna::VertexProcessor(art::Event const &e)
{
...
void VertexAna::SetupMaps(art::Event const &e) 
{
  MCPDGMap.clear();
  hitsMap.clear();
  mcCosmicMap.clear();
...
const art::Ptr<simb::MCParticle> VertexAna::GetMCParticle(int const &trackID)
{
  for(unsigned int part_i = 0; part_i < eHandleParticles->size(); ++part_i) {
    const art::Ptr<simb::MCParticle> particle(eHandleParticles,part_i);
...
const art::Ptr<recob::PFParticle> VertexAna::GetPrimaryPFP() 
{
  for(unsigned int pfp_i = 0; pfp_i < eHandlePFPs->size(); ++pfp_i) {
    const art::Ptr<recob::PFParticle> pfp(eHandlePFPs,pfp_i);
...
const art::Ptr<recob::PFParticle> VertexAna::GetPFP(unsigned int const &index) 
{
  for(unsigned int pfp_i = 0; pfp_i < eHandlePFPs->size(); ++pfp_i) {
    const art::Ptr<recob::PFParticle> pfp(eHandlePFPs,pfp_i);
...
const std::vector<art::Ptr<recob::PFParticle> > VertexAna::GetPrimaryNeutrinoPFPs() 
{
...
const std::vector<art::Ptr<recob::PFParticle> > VertexAna::GetSlicePrimary(std::vector<art::Ptr<recob::PFParticle> > const &slicePFPs)
{
  std::vector<art::Ptr<recob::PFParticle> > primaries;
...
const art::Ptr<simb::MCParticle> VertexAna::GetTrueParticle(std::vector<art::Ptr<recob::Hit> > const &trackHits)
{
...
const art::Ptr<simb::MCParticle> VertexAna::GetTrueShowerParticle(std::vector<art::Ptr<recob::Hit> > const &showerHits)
{
...
unsigned int VertexAna::HierarchyPrimary(art::Ptr<recob::PFParticle> const &pfp)
{
  bool regress = true;
...
float VertexAna::HitPurity(art::Event const &e, art::Ptr<recob::Shower> const &shower)
{
...
float VertexAna::HitPurity(art::Event const &e, art::Ptr<recob::Track> const &track)
{
...
float VertexAna::Completeness(art::Event const &e, art::Ptr<recob::Shower> const &shower)
{
  art::FindManyP<recob::Hit> showerHitsAssn(eHandleShowers,e,fShowerModuleLabel);
  std::vector<art::Ptr<recob::Hit> > showerHits = showerHitsAssn.at(shower.key());
...
float VertexAna::Completeness(art::Event const &e, art::Ptr<recob::Track> const &track)
{
  art::FindManyP<recob::Hit> trackHitsAssn(eHandleTracks,e,fTrackModuleLabel);
  std::vector<art::Ptr<recob::Hit> > trackHits = trackHitsAssn.at(track.key());
...
float VertexAna::TruedEdx(art::Ptr<simb::MCParticle> const &particle)
{
...
float VertexAna::TrueTrackLength(art::Ptr<simb::MCParticle> const &particle)
{
  float length = 0;
  unsigned int nTrajPoints = particle->NumberTrajectoryPoints();
...
float VertexAna::TrackLength(art::Ptr<recob::Track> const &track)
{
  float length = 0;
...
float VertexAna::MeanScatter(art::Ptr<recob::Track> const &track)
{
...
float VertexAna::StdDevScatter(art::Ptr<recob::Track> const &track, const float &meanScatter)
{
  float sumSqDiff = 0;
  int N = 0;
  int nTrajPoints = track->NumberTrajectoryPoints();
...
void VertexAna::TestingPlace(art::Event const& e) 
{
}

void VertexAna::ClearData()
{
  ...
  tr_theta.clear(); tr_length.clear(); tr_trackPFOScore.clear();
  // tr_chi2Proton.clear(); tr_chi2Pion.clear(); tr_chi2Muon.clear();
  tr_meanScatter.clear(); 
...
void VertexAna::analyze(art::Event const& e)
{
