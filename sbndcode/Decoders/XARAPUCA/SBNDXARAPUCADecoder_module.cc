...
: EDProducer{p}
{
  ...
  fframeshift_mode = p.get<bool> ("frameshift_mode", true);
...
void sbndaq::SBNDXARAPUCADecoder::produce(art::Event& e) {
  ...
  if (ftiming_priority == PTB_TIMING) {
    uint16_t hlt_code = HLT_NOT_FOUND;
...
/**
  ...
  * @param[in] timestamp The reference timestamp (ETRIG from the SPEC-TDC or HLT from the PTB) in nanoseconds.
  ...
  * SPEC-TDC, PTB, and CAEN-only timing. The calculated timestamps are returned in microseconds.
  *
  * @see get_spec_tdc_etrig_timestamp
  * @see get_ptb_hlt_timestamp
 */
...
bool sbndaq::SBNDXARAPUCADecoder::get_ptb_hlt_timestamp(art::Event& e, uint64_t corr_raw_timestamp, uint64_t& timestamp, uint16_t& hlt_code) {
  ...
  // The art::Handle object is valid and not empty.
    ...
    uint16_t hlt_type = HLT_NOT_FOUND;
