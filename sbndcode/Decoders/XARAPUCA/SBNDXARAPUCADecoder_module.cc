...
private:
  ...
  uint fevent_counter; /**< Event counter. */

  uint fnum_caen_boards; /**< Maximum number of CAEN boards to be considered. */
  uint ffragment_id_offset; /**< Offset value to get the slot of every CAEN board. */
  std::vector<uint> fboard_id_list; /**< Possible CAEN board identifiers based on the slot they are installed. */
  ...
  uint fns_per_sample; /**< Number of nanoseconds per sample. */
...
: EDProducer{p}
{
  ...
  fns_per_sample = p.get<uint> ("ns_per_sample");
  fns_per_tick = 8;
...
void sbndaq::SBNDXARAPUCADecoder::produce(art::Event& e)
{
  ...
  for (const std::string &caen_name: fcaen_fragment_names) {
    ...
    // The art::Handle object is valid and not empty.
      ...
      found_caen |= true;
      uint frag_handle_size = fragment_handle->size();
      ...
      if (fragment_handle->front().type() == artdaq::Fragment::ContainerFragmentType){
        ...
        for (size_t c = 0; c < frag_handle_size; c++) {
          ...
          if (container_fragment.fragment_type() == sbndaq::detail::FragmentType::CAENV1740) {
            uint num_caen_fragments = container_fragment.block_count(); 
  ...
  // otherwise it starts the decoding of the fragments found.
    ...
    for (size_t b = 0; b < fragments.size(); b++) {
      for (size_t f = 0; f < fragments[b].size(); f++) {
        ...
        uint TTT_end_ticks = header.triggerTime();
 ...
 * @param[in, out] fragments A nested std::vector of artdaq::Fragment objects to be decoded.
...
uint32_t sbndaq::SBNDXARAPUCADecoder::read_word(const uint32_t* & data_ptr) {
 ...
 /**
 * @brief Generates a unique channel identifier using the board slot and the channel number of that board.
...
uint sbndaq::SBNDXARAPUCADecoder::get_channel_id(uint board, uint board_channel) {
  uint channel_id = fboard_id_list[board] * 100 + board_channel;
  return channel_id;
}
