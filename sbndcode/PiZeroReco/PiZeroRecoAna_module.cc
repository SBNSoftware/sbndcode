...
// File:         PiZeroRecoAna_module.cc                                 //
// Author:       Henry Lay h.lay@lancaster.ac.uk                         //
//                                                                       //
// Purpose:      The module saves truth information regarding the        //
//               primary products of neutrino interactions, specifically //
//               those producing neutral pions. It also saves their      //
...
class PiZeroRecoAna : public art::EDAnalyzer {
public:
  explicit PiZeroRecoAna(fhicl::ParameterSet const& pset);

  PiZeroRecoAna(PiZeroRecoAna const&) = delete;
  PiZeroRecoAna(PiZeroRecoAna&&) = delete;
  PiZeroRecoAna& operator=(PiZeroRecoAna const&) = delete;
  PiZeroRecoAna& operator=(PiZeroRecoAna&&) = delete;
...
void PiZeroRecoAna::SetupMaps(art::Event const &e)
{
...
void PiZeroRecoAna::ReconstructionProcessor(art::Event const &e)
{
...
void PiZeroRecoAna::TruthProcessor(art::Event const &e)
{
...
std::vector<art::Ptr<recob::PFParticle> > PiZeroRecoAna::GetPrimaryPFPs(art::Event const &e)
{
  art::Handle<std::vector<recob::PFParticle> > handlePFPs;
  e.getByLabel(fPFParticleModuleLabel,handlePFPs);
...
art::Ptr<recob::PFParticle> PiZeroRecoAna::GetPFP(art::Event const &e, long unsigned int const &id)
{
  art::Handle<std::vector<recob::PFParticle> > handlePFPs;
  e.getByLabel(fPFParticleModuleLabel,handlePFPs);
...
float PiZeroRecoAna::Purity(std::vector< art::Ptr<recob::Hit> > const &objectHits, int const &trackID)
{
...
float PiZeroRecoAna::Completeness(std::vector< art::Ptr<recob::Hit> > const &objectHits, int const &trackID)
{
  std::map<int,int> objectHitsMap;
...
float PiZeroRecoAna::TrueTrackLength(art::Ptr<simb::MCParticle> const &particle)
{
  float length = 0;
...
void PiZeroRecoAna::analyze(art::Event const &e)
{
...
DEFINE_ART_MODULE(PiZeroRecoAna)
