...
void Hitdumper::analyze(const art::Event& evt)
{
  ...
  */
  ...
  if (nhits>kMaxHits) nhits=kMaxHits;
  ...
  // strips are always in pairs, one entry for each spim (2 sipms per strip) 
  for (int i = 0; i<nstr; i+=2){
    ...
    //
    // else if (tagger.first=="volTaggerFaceBack_0") ip=2; 
    // else if (tagger.first=="volTaggerSideLeft_0") ip=3; 
    // else if (tagger.first=="volTaggerSideRight_0") ip=4; 
    // else if (tagger.first=="volTaggerTopLow_0") ip=5; 
    // else if (tagger.first=="volTaggerTopHigh_0") ip=6; 
    // else if (tagger.first=="volTaggerBot_0") ip=0; 
