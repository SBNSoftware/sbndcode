...
: fRandomSeed(0), fLogLevel(1),
  ...
  m_pran(nullptr) {
  const string myname = "SBNDChannelNoiseService::ctor: ";
  ...
  string rname = "SBNDChannelNoiseService";
  if ( haveSeed ) {
...
: SBNDChannelNoiseService(pset) { }

//**********************************************************************

SBNDChannelNoiseService::~SBNDChannelNoiseService() {
  const string myname = "SBNDChannelNoiseService::dtor: ";
  if ( fLogLevel > 0 ) {
...
int SBNDChannelNoiseService::addNoise(Channel chan, AdcSignalVector& sigs) const {
...
ostream& SBNDChannelNoiseService::print(ostream& out, string prefix) const {
  out << prefix << "SBNDChannelNoiseService: " << endl;
...
generateMicroBooNoise(float wirelength, float ENOB, 
  ...
  const string myname = "SBNDChannelNoiseService::generateCoherentNoise: ";
  if ( fLogLevel > 1 ) {
...
generateGaussianNoise(AdcSignalVector& noise, std::vector<float> gausNorm, 
  ...
  const string myname = "SBNDChannelNoiseService::generateGaussianNoise: ";
  if ( fLogLevel > 1 ) {
...
generateCoherentNoise(AdcSignalVector& noise, std::vector<float> gausNorm, 
	                    ...
	                    TH1* aNoiseHist) const {
  const string myname = "SBNDChannelNoiseService::generateCoherentGaussianNoise: ";
  if ( fLogLevel > 1 ) {
...
void SBNDChannelNoiseService::makeCoherentGroupsByOfflineChannel(unsigned int nchpergroup) {
	CLHEP::RandFlat flat(*m_pran);
  CLHEP::RandGauss gaus(*m_pran);
	art::ServiceHandle<geo::Geometry> geo;
...
//**********************************************************************
unsigned int SBNDChannelNoiseService::getGroupNumberFromOfflineChannel(unsigned int offlinechan) const {
  return fChannelGroupMap[offlinechan];
}

//**********************************************************************
unsigned int SBNDChannelNoiseService::getCohNoiseChanFromGroup(unsigned int cohgroup) const {
...
DEFINE_ART_SERVICE_INTERFACE_IMPL(SBNDChannelNoiseService, ChannelNoiseService)
