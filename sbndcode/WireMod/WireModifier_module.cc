...
void WireModifier::produce(art::Event& evt) {
    ...
    // Pre-refactor sbncode interface: caf::PrepSimChannels returns pairs of
    // (WireID, IDE*) rather than sbn::ReadoutIDE. Only the wire and the IDE are
    // used below, so the pair carries everything this module needs.
    ...
    for (size_t i_w = 0; i_w < wire_vec.size(); i_w++) {
        ...
        for (size_t i_r = 0; i_r < nranges; i_r++) {
            ...
            for (size_t i_sr = 0; i_sr != sub_roi_properties.size(); i_sr++) {
                ...
                const std::vector<std::pair<geo::WireID, const sim::IDE*>> id_to_ide =
