...
// Input variables to define when reading in files
std::vector<TString> fFileNames;
std::vector<TString> fModelNames;
...
bool fPlotStacked;
TString fStackBy;
std::vector<std::string> fStackList;
...
const std::vector<int> fCols = {905, 867, 797, 835, 397, 879, 820, 633, 923};
...
const std::vector<int> fFillStyle = {3002, 3003, 3145, 3154, 3017, 3018, 3004, 3005, 3315, 3351, 3144, 3244, 3344, 3001};
const std::vector<std::string> fsi_list = {"0#pi0p", "0#pi1p", "0#pi#geq2p", "1#pi^{#pm}", "1#pi^{0}", "#geq2#pi^{#pm,0}", "other"};
...
class Titles
{
  public:
    ...
    Titles(std::vector<TString> ht, std::vector<TString> n, std::vector<TString> u, std::vector<TString> dt, std::vector<TString> mn, 
        TString pc, TString lc, TString ic, TString npr, TString npi, TString npi0, TString it, TString p, TString m)
    {
      hist_titles = ht;
      names = n;
      units = u;
...
void Configure(const std::string config_filename) {
  ...
  while(std::getline(config_file, line)){
    ...
    if(key.find("PlotAreaNorm") != std::string::npos)    fPlotAreaNorm = (value=="true");
    if(key.find("PlotFilled") != std::string::npos)  fPlotFilled = (value=="true");
    // Optional extras
    if(key.find("Show2DSlices") != std::string::npos)  fShow2DSlices = (value=="true");
...
void Init(){
  ...
  if(!fInputFile.Contains(".root")){

    ifstream files(fInputFile.Data());
    std::string line;
    ...
    for(const std::string &l : lines){
      // Make sure there is a space: Such that we can extract both a model name and
      // the file associated
      TString tL(l.data());
      if(!tL.Contains(" ")){
        std::cerr << " Error: Input file list must contain a space-separated list with the format:"
          << "\n <GENIEModelName> model_file_name.root"
          << "\n Note: Do not include spaces in the model name for obvious reasons"
          << std::endl;
        exit(1);
      }
      int pos0=0, pos1;
      TString modelName="", fileName="";
      while(l[pos0] != ' '){
        modelName += l[pos0];
        pos0++;
      }
      ...
      std::cout << " Reading input file generated with " << modelName << " model configuration" << std::endl;
      fModelNames.push_back(modelName);
      fFileNames.push_back(fileName);
    }
  }
  else{
    std::cout << " Notice: Since no file list was passed with model name specified,"
      << "\n reverting to default model name: G1810a0211a."
      << "\n If this is incorrect, pass the input file like so: "
      << "\n Filename: <InputFileName>.list || <InputFileName>.txt"
      << "\n Contents: <GENIEModelName> model_file_name.root"
      << std::endl;
    fFileNames.push_back(fInputFile);
    fModelNames.push_back("G1810a0211a");
  }
  assert(fModelNames.size() == fFileNames.size());

}

// Check if the fiducial volume has been defined
bool IsFiducial(){
...
std::map< TString, std::map< TString, std::vector<Interaction> > > ReadData(){

  // Now setup the objects as a map between models, stages and interactions

  std::map< TString, std::map< TString, std::vector<Interaction> > > modelInteractions;

  // Loop over files and read
  for(unsigned int i = 0; i < fFileNames.size(); ++i){
    //Get the model name for the map
    TString m = fModelNames.at(i);
    TString f = fFileNames.at(i);

    // Open the root tree
    TFile data_file(f.Data());
    if(!data_file.IsOpen()){
    ...
    for(const TString &s : fStage){
      ...
      while (tree_reader.Next()) {
        ...
        if(IsFiducial()){
          ...
          if(*true_n_pi0 == 1 && *true_n_pipm == 0) fsi_string = "1#pi^{0}";
          else if(*true_n_pipm == 1 && *true_n_pi0 == 0) fsi_string = "1#pi^{#pm}";
          else if(*true_n_pipm+*true_n_pi0 >= 2) fsi_string = "#geq2#pi^{#pm,0}";
    ...
    modelInteractions.emplace(m,interactions);
  } // models
...
void GetMetaData(){
  ...
  //Get geometry information which should be universal to all models read from the files
  ...
  for(unsigned int i = 0; i < fFileNames.size(); ++i){
    ...
    if(fSinglePot)
      pot_count = pot_count / static_cast<double>(tree_reader.GetEntries());

    if(fPotScale > 0){
      fPotScaleFac.push_back(fPotScale/pot_count);
    }
...
std::vector<std::vector<double>> GetBinning(std::vector<std::vector<double>> data_v){
  ...
  for(size_t i = 0; i < fPlotVariables.size(); i++){
    ...
    if(fNumBins[i] <= 0){
      ...
      hist_bins[i] = DefaultBins(data_size*fPotScaleFac.at(0), data_map.size(), i);
    }
  }
  ...
  if(fMaxError > 0){
    for(size_t i = 0; i < fPlotVariables.size(); i++){
      ...
      for(size_t n = 1; n <= temp_hist->GetNbinsX(); n++){
        temp_hist->SetBinContent(n, temp_hist->GetBinContent(n)*fPotScaleFac.at(0));
      }
...
Titles GetTitles(){
  ...
  Titles titles(hist_titles, names, units, data_type, fModelNames, part_cont, lep_cont, is_cc, n_pr, 
...
void DrawInfo(Titles titles, double width, double height, double size, int modelIt){
  ...
  for(const TString &type : titles.data_type){
    TLatex *temp_data_type = new TLatex(width, .86*height, type);
...
void Plot1DHistWithErrors(TH1D* error_bands, Titles titles, TH1D* total_hist, int modelIt, int i, int j = -1, int k = -1){
  ...
  double title_size = total_hist->GetYaxis()->GetTitleSize();
  ...
  total_hist->GetYaxis()->SetLabelSize(0.9*title_size);
  ...
  // Set axis titles
  error_bands->SetLineColor(fCols[1]);
  error_bands->SetLineStyle(fLineStyle[1]);
  if(fPlotFilled){
    error_bands->SetFillColor(fCols[1]);
    error_bands->SetFillStyle(fFillStyle[1]);
  ...
  // x axis config
  error_bands->GetXaxis()->SetTitleSize(size_ratio*total_hist->GetXaxis()->GetTitleSize());
  error_bands->GetXaxis()->SetLabelSize(size_ratio*total_hist->GetXaxis()->GetLabelSize());
  error_bands->GetXaxis()->SetLabelOffset(0.005);
  error_bands->GetXaxis()->SetTickLength(size_ratio*0.04);
  error_bands->SetTitleOffset(1.2, "x");
  // y axis config
  error_bands->GetYaxis()->SetTitleSize(size_ratio*total_hist->GetYaxis()->GetTitleSize());
  error_bands->GetYaxis()->SetLabelSize(size_ratio*total_hist->GetYaxis()->GetLabelSize());
  error_bands->GetYaxis()->CenterTitle();
  error_bands->GetYaxis()->SetTickLength(0.015);
  error_bands->SetNdivisions(105, "y");
  error_bands->SetTitleOffset(0.8, "y");
...
void Plot1DWithErrors(THStack* hstack, TLegend* legend, TH1D* error_bands, Titles titles, TH1D* total_hist, int modelIt, int i, int j = -1, int k = -1){
  ...
  canv_name.ReplaceAll("-","m");
  canvas->SetName(canv_name);
  output_file.ReplaceAll(".","_"+name+"_stacked.");
  canvas->SaveAs(output_file);
  output_file.ReplaceAll(".root",".png");
  canvas->SaveAs(output_file);
  output_file.ReplaceAll(".png",".pdf");
...
void Plot1DHist(Titles titles, TH1D* total_hist, int modelIt, int i, int j = -1, int k = -1){
  ...
  // X axis config
  total_hist->GetXaxis()->SetTitleOffset(1.1);
  total_hist->GetXaxis()->SetTickLength(0.02);
  total_hist->GetXaxis()->SetTitleSize(0.05);
  total_hist->GetXaxis()->SetLabelSize(0.035);
  
  // Y axis config
  total_hist->GetYaxis()->SetTitleOffset(1.2);
  total_hist->GetYaxis()->SetTickLength(0.015);
  total_hist->GetYaxis()->SetTitleSize(0.05);
...
void PlotOverlay1DWithErrors(std::map< TString, std::map<TString, TH1D*> > histograms, std::map< TString, std::map<TString, TH1D*> >  errors, Titles titles, TString title, int i, int j = -1, int k = -1){
  ...
  for(it_models = histograms.begin(); it_models != histograms.end(); ++it_models){
    ...
    for(it = stage_hists.begin(); it != stage_hists.end(); ++it){
      ...
      {
        total_hist->GetYaxis()->SetTitleOffset(1.45);
        total_hist->GetYaxis()->SetTitleSize(0.9*title_size);
        if(fPlotVariables.size()==1)
          canvas->Modified();
      }
  ...
  index = 0;
  std::map< TString, std::map<TString, TH1D*> >::const_iterator ite;
  std::map<TString, TH1D*>::const_iterator stage_ite;
  double max_y_err = -1.;
  for(ite = errors.begin(); ite != errors.end(); ++ite){
    TString model = ite->first;
    std::map< TString, TH1D*> stage_errors = ite->second;
    unsigned int stage_index = 0;
    for(stage_ite = stage_errors.begin(); stage_ite != stage_errors.end(); ++stage_ite){
      TString stage     = stage_ite->first; 
      TH1D *error_bands = stage_ite->second;
      ...
      error_bands->GetXaxis()->SetTitleSize(0.9*size_ratio*histograms.begin()->second.begin()->second->GetXaxis()->GetTitleSize());
      error_bands->GetXaxis()->SetLabelSize(size_ratio*histograms.begin()->second.begin()->second->GetXaxis()->GetLabelSize());
      error_bands->GetXaxis()->SetTickLength(size_ratio*0.04);
      error_bands->SetTitleOffset(1.2, "x");
      // y axis config
      ...
      error_bands->GetYaxis()->SetTitleSize(size_ratio*histograms.begin()->second.begin()->second->GetXaxis()->GetTitleSize());
      error_bands->GetYaxis()->SetLabelSize(size_ratio*histograms.begin()->second.begin()->second->GetXaxis()->GetLabelSize());
      ...
      error_bands->GetYaxis()->SetTickLength(0.015);
      error_bands->SetNdivisions(105, "y");
      error_bands->SetTitleOffset(0.4, "y");

      // Draw the error bars
      if(stage_ite == stage_errors.begin()){
  ...
  canvas->SetName(canv_name);
  TString output_file = fOutputFile;
  output_file.ReplaceAll(".","_"+var+"_overlay_errors.");
  output_file.ReplaceAll(".root","");
  output_file.ReplaceAll(".","p");
  output_file.ReplaceAll("-","m");
  output_file.Append(".root");
...
void PlotEfficiency(std::map< TString, std::map<TString, TH1D*> > select, std::map< TString, std::map<TString, TH1D*> > total, TString name, TString xaxis, TString yaxis){

  // If there are multiple stages, draw a legend
  bool multiple_stages = false;
  if(select.size() + select.begin()->second.size() > 2)
    multiple_stages = true;

  std::map< TString, std::map<TString, TH1D*> >::const_iterator model_it;
  std::map<TString, TH1D*>::const_iterator it;
  unsigned int index = 0;
  for(model_it = select.begin(); model_it != select.end(); ++model_it){
    TString mo = model_it->first;
    std::map<TString, TH1D*> stage_select = model_it->second;
    std::map<TString, TH1D*> stage_total  = total.at(mo);
    unsigned int stage_index = 0;
    for(it = stage_select.begin(); it != stage_select.end(); ++it){
      TString st = it->first;
      ...
      output_file.ReplaceAll(".","_"+name+".");
      output_file.ReplaceAll(".","_"+mo+"_"+st+".");

      TLegend *l = new TLegend(0.68,0.18,0.88,0.38);

      graph->SetMarkerColor(46);
      graph->SetLineColor(46);
      graph->SetLineStyle(fLineStyle[stage_index]);
      ...
      graph->BayesDivide(stage_select.at(st), stage_total.at(st));
      if(multiple_stages){
        if(fStage.size() == 1)
          l->AddEntry(graph, mo, "lf");
        else if(fModelNames.size() == 1)
          l->AddEntry(graph, st, "lf");
        else
          l->AddEntry(graph, mo+" "+st, "lf");
      }
      if(index+stage_index == 0)
        graph->Draw("ap");
      else
        graph->Draw("ap same");
...
void Plot2D(TH2D* hist, TString name, TString xaxis, TString yaxis){
  ...
  canvas->SetBottomMargin(0.12);
  canvas->SetLeftMargin(0.098);
  canvas->SetRightMargin(0.13);

  // Scale for XSec if needed
  if(fPlotXSec){
    for(unsigned int i = 1; i <= hist->GetNbinsX(); ++i){
      double width_x = hist->GetXaxis()->GetBinWidth(i);
      for(unsigned int j = 1; j <= hist->GetNbinsY(); ++j){
        double width_y      = hist->GetYaxis()->GetBinWidth(j);
        double scale_factor = fXSecScale / static_cast<double>(width_x * width_y);
        double old_content  = hist->GetBinContent(i,j);
        double old_error    = hist->GetBinError(i,j);
        double new_content  = old_content*scale_factor;
        double new_error    = old_error*scale_factor;
        hist->SetBinContent(i,j,new_content);
        hist->SetBinError(i,j,new_error);
      }
    }
  }
  ...
  TString output_file = fOutputFile;
  output_file.ReplaceAll(".","_"+name+"_2D.");
  canvas->SaveAs(output_file);
  output_file.ReplaceAll(".root",".png");
  canvas->SaveAs(output_file);
  output_file.ReplaceAll(".png",".pdf");
  ...
  if(fSaveAllInOne){
    TFile f(fOutputFile, "UPDATE");
    hist->Write(name+"_2D");
    canvas->Write("canv_"+name+"_2D");
    f.Close();
  }
}

// Create stacked histogram and legend from data
std::pair<THStack*, TLegend*> StackHist1D(std::map<std::string, std::vector<std::vector<double>>> data, const double scale, TString name, TString title, std::vector<std::vector<double>> bin_edges, int i, int j = -1, int bin_j = -1, int k = -1, int bin_k = -1){
  ...
  int index = 0;
  for(const std::string &l : fStackList){
    for(auto const& dat: data){
      if(dat.first != l)  continue;
...
TH1D* GetTotalHist(std::vector<std::vector<double>> data, TString name, TString title, std::vector<std::vector<double>> bin_edges, const double scale, int i, int j = -1, int bin_j = -1, int k = -1, int bin_k = -1){

  double edges_array[bin_edges[i].size()];
  std::copy(bin_edges[i].begin(), bin_edges[i].end(), edges_array);
  TH1D *total_hist = new TH1D("total_"+name, title, bin_edges[i].size()-1, edges_array);
...
void PlotEffPur(std::map< TString, std::map< TString, std::vector<Interaction> > > interactions, TString name, Titles titles, std::vector<std::vector<double>> bin_edges, int i, int j = -1, int bin_j = -1, int k = -1, int bin_k = -1){

  double edges_array[bin_edges[i].size()];
  std::copy(bin_edges[i].begin(), bin_edges[i].end(), edges_array);

  std::map< TString, std::map<TString, TH1D*> > model_eff_numerator, model_eff_denom, model_pur_numerator, model_pur_denom;

  for(const TString &m : fModelNames){
    std::map<TString, TH1D*> eff_numerator, eff_denom, pur_numerator, pur_denom;
    for(const TString &s : fStage){
      if(s != "reco" && s != "smeareff") {
        std::cout << " Can't plot efficiency and purity for anything other than reco or smeareff stages" << std::endl;
        continue;
      }
    ...
    model_eff_numerator.emplace(m,eff_numerator);
    model_eff_denom.emplace(m,eff_denom);
    model_pur_numerator.emplace(m,pur_numerator);
    model_pur_denom.emplace(m,pur_denom);
  }

  if(titles.units[i].IsNull()){
    // Efficiency: selected/total in true
    PlotEfficiency(model_eff_numerator, model_eff_denom, name, titles.names[i]+"^{true}", "Efficiency");
    // Purity: correct selected/total selected
    PlotEfficiency(model_pur_numerator, model_pur_denom, name, titles.names[i]+"^{reco}", "Purity");
  }
  else{
    // Efficiency: selected/total in true
    PlotEfficiency(model_eff_numerator, model_eff_denom, name, titles.names[i]+"^{true} ["+titles.units[i]+"]", "Efficiency");
    // Purity: correct selected/total selected
    PlotEfficiency(model_pur_numerator, model_pur_denom, name, titles.names[i]+"^{reco} ["+titles.units[i]+"]", "Purity");
  }

  /*
  for(const TString &m : fModelNames){
    for(const TString &s : fStage){
      if(model_eff_numerator.at(m).at(s)) delete model_eff_numerator.at(m).at(s); 
      if(model_eff_denom.at(m).at(s)) delete model_eff_denom.at(m).at(s);
      if(model_pur_numerator.at(m).at(s)) delete model_pur_numerator.at(m).at(s);
      if(model_pur_denom.at(m).at(s)) delete model_pur_denom.at(m).at(s);
    }
  }*/
}
...
void PhysicsBookPlots(std::string config = "config.txt"){
  ...
  std::cout << "Initialising and reading MetaData...\n";
  Init();
  GetMetaData();
  std::cout<<"...Finished.\n";
  ...
  for(const TString &m : fModelNames){
    ...
    for(const TString &s : fStage){
      for(auto const& in : interactions.at(m).at(s)){
        ...
        model_total_data[s].push_back(in.variables);
        if(!fPlotStacked){
          model_stack_data[s]["all"].push_back(in.variables);
        }
        else if(fStackBy == "fsi"){
          model_stack_data[s][in.fsi].push_back(in.variables);
          fStackList = fsi_list;
        }
        else if(fStackBy == "int"){
          model_stack_data[s][in.int_type].push_back(in.variables);
          fStackList = int_list;
        }
        else if(fStackBy == "nu"){
  ...
  std::vector<std::vector<double>> bin_edges = GetBinning(total_data.at(fModelNames.at(0)).at(fStage.at(0)));
  std::cout<<"...Finished.\n";
  ...
  if(fShow2DSlices){
    ...
    n_hists = fPlotVariables.size();
  }
  n_hists *= (fStage.size()*fModelNames.size());

  // Ask user if they want to make that many histograms
  std::string response = "y";
  if(n_hists>10){
    std::cout<<"This will produce "<<n_hists<<" histograms for all stages, models and kinematic slices, continue (y/n)? ";
    std::cin>>response;
  }
  ...
  for(size_t d_i = 0; d_i < fPlotVariables.size(); d_i++){
    ...
    for(const TString &m : fModelNames){
      if(mi > fPotScaleFac.size()) {
        std::cerr << " Error: Trying to access entry " << mi << " of model names list, but list has only got: " << fModelNames.size() << std::endl;
        std::exit(1);
      }
      std::map<TString, TH1D*> stage_total_hist, stage_error_band;
      for(const TString &s : fStage){
        TString name_1D_s = name_1D+"_"+m+"_"+s;
        TString title_1D_s = title_1D+" "+m+" "+s;
        stage_total_hist.emplace(s, GetTotalHist(total_data.at(m).at(s), name_1D_s, title_1D_s, bin_edges, fPotScaleFac.at(mi), d_i));
        stage_error_band.emplace(s, GetErrorBand(stage_total_hist.at(s), name_1D_s, bin_edges, d_i));

        // Create a total 1D stacked histogram for each of the variables
        std::pair<THStack*, TLegend*> stack = StackHist1D(stack_data.at(m).at(s), fPotScaleFac.at(mi), name_1D_s, title_1D_s, bin_edges, d_i);

        // Draw the plots
        if(fShowStatError){
          if(fPlotStacked)
            Plot1DWithErrors(stack.first, stack.second, stage_error_band.at(s), titles, stage_total_hist.at(s), mi, d_i);
          else
            Plot1DHistWithErrors(stage_error_band.at(s), titles, stage_total_hist.at(s), mi, d_i);
        }
        else{
          if(fPlotStacked)
            Plot1D(stack.first, stack.second, titles, stage_total_hist.at(s), mi, d_i);
          else
    ...
    for(size_t d_j = 0; d_j < fPlotVariables.size(); d_j++){
      ...
      std::map< TString, std::map< TString, std::map< int, TH1D*> > > slice_bin_total_hists, slice_bin_error_bands;
      unsigned int mi = 0;
      for(const TString &m : fModelNames){
        for(const TString &s : fStage){
          TString name_1D_s = name_1D+"_"+m+"_"+s;
          ...
          for (int n = 0; n < total_data.at(m).at(s).size(); n++){
            hist_2D->Fill(total_data.at(m).at(s)[n][d_i], total_data.at(m).at(s)[n][d_j]);
          }
          ...
          Plot2D(hist_2D, fPlotVariables[d_i]+"_"+fPlotVariables[d_j]+"_"+m+"_"+s, plot_label_var1, plot_label_var2);

          if(!fShow2DSlices) continue;

          if(bin_edges[d_j].size() > 20){
            std::cout << " There are way too many bin edges to slice " << fPlotVariables[d_j] << ", skipping" << std::endl;
            continue;
          }
          for(int bin_j = 0; bin_j < bin_edges[d_j].size()-1; bin_j++){
            ...
            if(fMaxError>0){
              std::vector<double> bin_edges_new = ChangeBinning2D(total_data.begin()->second.begin()->second, bin_edges, d_i, d_j, bin_j);
              bin_edges_copy[d_i] = bin_edges_new;
            }
            // Get the file name and title of the histogram
            TString name_2D = fPlotVariables[d_i] +"_"
              + fPlotVariables[d_j] +"_"+ Form("%.1f", bin_edges_copy[d_j][bin_j]) +"_"+ Form("%.1f", bin_edges_copy[d_j][bin_j+1]) + "_" + m + "_" + s;
            TString title_2D = titles.hist_titles[d_j] 
              +": ["+ Form("%.2f", bin_edges_copy[d_j][bin_j]) +", "+ Form("%.2f", bin_edges_copy[d_j][bin_j+1]) +"] " + m + " " + s;


            // Only make these plots for 2 variables
            if(fPlotVariables.size() != 2) continue;
            TH1D *total_hist_2D = hist_2D->ProjectionX(name_2D,bin_j,bin_j+1);
            TH1D *error_band_2D = GetErrorBand(total_hist_2D, name_2D, bin_edges_copy, d_j);
            slice_bin_total_hists[m][s][bin_j] = total_hist_2D;
            slice_bin_error_bands[m][s][bin_j] = error_band_2D;

            total_hist_2D->SetTitle(title_2D);
            // Draw the plots
            if(fShowStatError){
              Plot1DHistWithErrors(error_band_2D, titles, total_hist_2D, mi, d_i);
            }
            else{
              Plot1DHist(titles, total_hist_2D, mi, d_i);
            }
          } // Binsj
        } // Stages
        mi++;
      } // Models
      if(!fShow2DSlices || (fStage.size()+fModelNames.size() == 1)) continue;
      ...
      for(int bin_j = 0; bin_j < bin_edges[d_j].size()-1; bin_j++){
        std::map<TString, std::map< TString, TH1D* > > sliced_total_hists, sliced_error_bands;
        for(const TString &m : fModelNames){
          for(const TString &s : fStage){
            sliced_total_hists[m][s] = slice_bin_total_hists.at(m).at(s).at(bin_j);
            sliced_error_bands[m][s] = slice_bin_error_bands.at(m).at(s).at(bin_j);
            TString total_hist_slice_name(sliced_total_hists.at(m).at(s)->GetName()); 
            TString error_band_slice_name(sliced_error_bands.at(m).at(s)->GetName());
            total_hist_slice_name += ("_"+std::to_string(bin_j));
            error_band_slice_name += ("_"+std::to_string(bin_j));
            total_hist_slice_name.ReplaceAll(".","p");
            error_band_slice_name.ReplaceAll(".","p");
            sliced_total_hists.at(m).at(s)->SetName(total_hist_slice_name);
            sliced_error_bands.at(m).at(s)->SetName(error_band_slice_name);
          } // Stage
        } // Model
        if(fShowStatError)
          PlotOverlay1DWithErrors(sliced_total_hists, sliced_error_bands, titles, "sliced_overlay", d_i, d_j);
        else
          PlotOverlay1D(sliced_total_hists, titles, "sliced_overlay", d_i, d_j);
      } // Binj
    } // Variable j
  } // Variable i
  std::cout<<"...Finished.\n";
