...
namespace lcvn {
  ...
  void SBNDCVNZlibMaker::beginJob()
  {
    ...
    /*if(fSaveTree){
       ...
       fEventTree->Branch("IsSliceNu", &fIsSliceNu);
    }*/
  }
  ...
  {
       ///////////////////////////// Save TTree information ////////////////////////////////////////
       //std::cout << "********* " << evt.run() << "  " << evt.subRun() << "  " << evt.id().event() << "  **************\n";     	  
	  ...
	  art::FindManyP<recob::PFParticle> fm_slicepfp(SliceListHandle, evt, fSliceLabel);
	  
	  //std::cout << "Number of slices : " << SliceList.size() << "\n";
	  ...
	  for(auto const &slice : SliceList){
	      ...
	      fIsSliceNu.push_back(false);
	      if(fm_slicepfp.isValid()){
	         std::vector<art::Ptr<recob::PFParticle>> slicePFPs = fm_slicepfp.at(slice.key());
		 //std::cout << "Slice number : " << slice->ID() << "  Number of pfparticles in the slice : " << slicePFPs.size() << "\n";
		 
		 for(auto const &pfp : slicePFPs){
		     if((pfp->PdgCode() == 12 || pfp->PdgCode() == 14)){
		         fIsSliceNu.back() = true;
	  ...
	  if(nuPfpVec.size()){
		 ...
		 if(fm_slicepfp.isValid()){
		    ...
		    std::cout << "size of the meta vector : " << pfpMetaVec.size() << "\n";
	         }
	  ...
	  if(fUseBackTrackInfo){
		 ...
		 frun = evt.run();
		    ...
		    //////////////////////// TEST ////////////////////////////////////// 	 
		     ...
		     /*if(evt.run() == 1 && evt.subRun() == 18 && evt.id().event() == 14){
		        std::vector<art::Ptr<recob::Hit>> slice_hits = findManyHits.at(SliceList[j].key());
			std::cout << "========== Slice ID : " << SliceList[j]->ID() << "  Number of hits in the slice : " << slice_hits.size() << "\n";
		      }	*/ 
...
/*bool SBNDCVNZlibMaker::Is_in_TPC(double stX, double stY, double stZ)
{
