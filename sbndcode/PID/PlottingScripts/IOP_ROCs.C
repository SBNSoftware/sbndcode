...
void IOP_ROCs(const std::vector<PIDTraining> &trainings, const TString save_file_suffix,
              const int pdg, const int motherPDG = -1, const double purThresh = 0.5, const double compThresh = 0.5);

void IOP_ROCs()
{
  IOP_ROCs(clean_v12_trainings, "electron", 11);
  IOP_ROCs(clean_v12_trainings, "muon", 13);
  IOP_ROCs(clean_v12_trainings, "photon", 22);
  IOP_ROCs(clean_v12_trainings, "pizero_photon", 22, 111);
  IOP_ROCs(clean_v12_trainings, "pion", 211);
  IOP_ROCs(clean_v12_trainings, "proton", 2212);

  IOP_ROCs(clean_v12_trainings, "electron_high_quality", 11, -1, 0.8, 0.8);
  IOP_ROCs(clean_v12_trainings, "muon_high_quality", 13, -1, 0.8, 0.8);
  IOP_ROCs(clean_v12_trainings, "photon_high_quality", 22, -1, 0.8, 0.8);
  IOP_ROCs(clean_v12_trainings, "pizero_photon_high_quality", 22, 111, 0.8, 0.8);
  IOP_ROCs(clean_v12_trainings, "pion_high_quality", 211, -1, 0.8, 0.8);
  IOP_ROCs(clean_v12_trainings, "proton_high_quality", 2212, -1, 0.8, 0.8);
}

void IOP_ROCs(const std::vector<PIDTraining> &trainings, const TString save_file_suffix,
              const int pdg, const int motherPDG, const double purThresh, const double compThresh)
{
  const TString save_dir = "/exp/sbnd/data/users/hlay/ncpizero/plots/NCPiZeroAv12/razzled/iop_rocs";
  ...
  for(auto const& training : trainings)
    {
      ...
      TH1F *hSignalPFP     = new TH1F("hSignalPFP" + training.name,
                                      ";" + training.printed_name + " " + pdgStrings.at(pdg) + " Score;Reconstructed Particles (A.U.)",
                                      1200, -0.1, 1.1);
      ...
      hBackgroundPFP->Rebin(25);
      hSignalPFP->Rebin(25);

      hBackgroundPFP->GetYaxis()->SetTitleOffset(1.15);
      hBackgroundPFP->GetYaxis()->SetTitleSize(0.06);
  ...
  multi->SetTitle(Form(";%s Efficiency;Background Rejection", pdgStrings.at(pdgKey).Data()));
  multi->Draw("AC");
