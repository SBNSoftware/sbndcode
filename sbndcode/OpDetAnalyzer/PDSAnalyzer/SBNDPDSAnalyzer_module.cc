...
void opdet::SBNDPDSAnalyzer::analyze(art::Event const& e)
{
  ...
  if(fSaveRawWaveforms){
    ...
    std::cout << " Before Getting the waveform labels " << std::endl;
    e.getByLabel(fRawWaveformsModuleLabel, wvfHandle);
    std::cout << " After Getting the waveform labels " << std::endl;
    ...
    _opChDigi.clear();
    if(fVerbosity>0)
      std::cout << "Before saving raw waveforms " << std::endl;
    for(auto const& wvf : (*wvfHandle)) {
      int fChNumber = wvf.ChannelNumber();
      if(fChNumber>305) continue;
    ...
    if(fVerbosity>0)
      std::cout << "After saving raw waveforms " << std::endl;
